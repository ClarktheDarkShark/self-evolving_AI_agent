...
- Complete The Mechanical Work: The macro MUST actually compute the final required state for each candidate (e.g., executing the full intersection loop, counting, or sorting). If it returns raw, unprocessed first-hop variables without finishing the mechanical work, penalize it heavily (-4 grade).
...
- ATTRIBUTE FILTERING EXECUTION (CRITICAL):
  * If a task includes attribute literals (e.g., "semi-firm"), macros MUST attempt the reverse-neighbor hop (`get_relations` on literal -> `get_neighbors` -> `intersection`) before delegating.
  * If the macro delegates attribute filtering without making bounded live attempts, penalize heavily (-4 grade).
  * If bounded live attempts are made and the literal path is blocked/error/empty, do NOT penalize delegation; this is acceptable if clearly reported.
- RICH OBSERVATIONS UNDER NOISE (CRITICAL): You MUST NOT penalize macros for returning a rich mapping of candidates (e.g., `{"Candidate 1": "#1", "Candidate 2": "#2"}`) when they completed the required mechanical work and a single final variable is not reliable due to KG ambiguity/noise.
...
7. RELATION-NAME INTERSECTION ON MULTIPLE SOURCES (-4 Grade):
When a task has multiple source nodes (e.g., "Goat" AND "cows"), intersecting relation NAME STRINGS across sources with Python set intersection (e.g., `set_A & set_B` or `.intersection(...)`) is the wrong algorithm. Heterogeneous KG entities belong to different ontological namespaces and will almost never share an exact relation name, so this produces an empty candidate set, zero calls to get_neighbors, and a null/empty result. The correct algorithm is: for EACH source independently, take top-3 tier-1-filtered relations, call get_neighbors for each, then find cross-source intersections of the resulting NEIGHBOR VARIABLE SETS via `actions_spec["intersection"]`. Apply -4 grade if the tool intersects raw relation name strings across multiple sources instead of intersecting neighbor variable sets.
""").strip()
...
MACRO PAYLOAD (HARD)
- If the selected tool is a MACRO tool, you MUST include `"entities": [...]`, `"target_concept": "..."`, and `"domain_hints": [...]` in the payload.
...
- `domain_hints`: A JSON array of 1 to 3 short strings representing the high-level semantic domains/categories of the query (e.g., "food", "medicine", "music", "biology", "fictional_universe"). You must deduce this strictly from the user's instruction.
- Example task: "what semi-firm textured cheese is made from the products of goat and cows?"
- Example output: {"tool_name": "multi_entity_intersection_macro_tool", "payload": {"entities": ["goat", "cows", "semi-firm"], "target_concept": "cheese", "domain_hints": ["food", "biology"]}}
...
1. If you attempt an `intersection(#X, #Y)` and receive an environment error stating "Two Variables must have the same type", DO NOT call the macro again.
...
3. These events mean the variables are either semantically incompatible or require manual routing. Your immediate next action MUST be to pivot to manual exploration.
4. Use `get_relations(<Entity Name>)` or `get_attributes(#X)` on the provided entities or partial variables to manually explore the graph, find the correct relation paths, and deduce the answer yourself.
...
1. RICH OBSERVATION HANDLING:
If a tool returns a dictionary of candidate variables mapped to different relations (e.g., `{"cheese_milk_source": "#3", "common.topic": "#4"}`), you MUST read the relation names, semantically select the one that best matches the prompt's intent, and proceed with that specific variable.

2. THE REVERSE-NEIGHBOR ATTRIBUTE FILTER:
If you have a base variable (e.g., `#3` for 'cheeses') and an unapplied/blocked attribute (e.g., 'semi-firm'), you must manually filter the set.
  - Step 1: `Action: get_relations(semi-firm)` -> returns relations (e.g., `food.cheese_texture.cheeses`).
  - Step 2: `Action: get_neighbors(semi-firm, food.cheese_texture.cheeses)` -> returns a new variable (e.g., `#4`).
  - Step 3: `Action: intersection(#3, #4)` -> returns the final filtered variable.
...
- RESTRICTED PYTHON NAMESPACE: Your generated code executes in a strict Python environment. Do not use built-in functions like `reversed()`, `enumerate()`, `zip()`, `eval()`, or `exec()`. Stick to basic `for` loops and standard list indexing (e.g., `list[::-1]`).
...
```
- ALWAYS call `clean_env_output(...)` before extracting relations, variable IDs, or counts.
- NEVER treat prefixes like `"... executes successfully. Observation:"` as data tokens.
- For cleaned relation payloads that look like `[a, b, c]`, parse with a small deterministic splitter (`strip("[]")`, split on commas, per-token `.strip()`), then filter noise.
...
RELATION DISCOVERY & NOISE FILTERING (CRITICAL):
1. Call `get_relations_fn(source)` for all your identified source nodes.
2. SANITIZE: After comma-splitting (see Parsing Rule above), `.strip()` every token. Filter out any token that starts with `"http"` or contains `"/"`. WARNING: This filter only works correctly if you used comma-split — NOT character-class regex. The regex `[A-Za-z0-9_.\-]+` splits `"http://rdf.freebase.com/key/en"` into `"rdf.freebase.com"`, `"key"`, `"en"`. These fragments have no `http` prefix and no `/`, so they silently pass your filter and contaminate the intersection with fake "common" relations, producing a wrong answer.
3. Find candidate relations — TWO PATHS based on how many source nodes exist:
   ...
   - MULTIPLE SOURCES — NEIGHBOR-SET INTERSECTION (NOT relation-name intersection):
     CRITICAL: Heterogeneous KG entities (e.g., "Goat", "cows") belong to different ontological namespaces and will almost never share an exact relation name. If you set-intersect relation NAME STRINGS across sources, you get an empty set, zero candidates, and a null result. You MUST use PER-SOURCE INDEPENDENT EXPLORATION instead:
     a) For EACH source node independently, apply tier-1 filtering (see step 4 below) to its sanitized relations and take the top-3 clean relations (hard cap to prevent DB overload).
     b) For EACH (source, relation) pair, call `get_neighbors_fn(source, rel)` to mint one neighbor variable string per pair.
     c) CROSS-SOURCE INTERSECTION: For each combination of one neighbor variable from source_A paired with one from source_B (and source_C if present), call `actions_spec["intersection"]` on those variable strings.
     d) Collect non-empty intersections into `final_candidates` dict: key = "rel_A × rel_B [× rel_C...]", value = intersection_variable_string.
     e) If every combination yields an empty intersection after exhausting all combinations, do NOT return `status="blocked"` — apply GRACEFUL DEGRADATION instead: return `status="done"` with the individual base variables (keyed with their relation names) so the Solver can continue manually.
     → For MULTIPLE SOURCES, steps 4 and 5 are replaced by (b)-(d) above. Do NOT call get_neighbors_fn again after this step.

STRICT DOMAIN ADHERENCE:
When discovering candidate relations to intersect, you MUST heavily prioritize relations that match the `domain_hints` provided in the payload. Do not arbitrarily intersect numerical attributes or generic structural relations (such as `average_molar_mass` or `contained_by`) just because they happen to exist for both entities, unless they explicitly align with the semantic intent of the query. If no domain-relevant intersection exists after exhausting domain-hinted relations, degrade gracefully (see GRACEFUL DEGRADATION) rather than performing math on semantically unrelated properties.

4. [SINGLE SOURCE ONLY] DYNAMIC NAMESPACE FILTERING (CRITICAL, TWO-TIER): Knowledge Graphs contain massive amounts of generic structural noise. You MUST apply this exact two-tier filtering logic to your candidate relations:
  ...
  You MUST NEVER fall back to the raw unfiltered relations list. `common.` and `type.` relations must never survive.
5. [SINGLE SOURCE ONLY] Call `get_neighbors_fn(source, relation)` for your `final_candidates`.
...
- ATTRIBUTE LITERAL EXECUTION (CRITICAL): If an attribute literal is identified (e.g., "semi-firm"), you MUST attempt the reverse-neighbor attribute hop inside the Macro:
  1) `get_relations_fn(attribute_literal)`,
  2) choose viable non-noise relation(s),
  3) `get_neighbors_fn(attribute_literal, rel)` to mint attribute-constrained variable(s),
  4) intersect with your base candidate variable(s).
  Do NOT delegate attributes by default.
- CONDITIONAL DELEGATION ONLY IF BLOCKED: You may leave attributes unresolved ONLY if live calls for that literal are blocked/error/empty after bounded attempts. If this happens, include a short blocked reason in `answer_recommendation`.
- THE VISIBILITY RULE: Downstream agents cannot read the raw JSON dict. You must format your output into `answer_recommendation` with explicit text. Examples by outcome:
  - Full result:  `"Macro complete. Final Result: #7. Candidate map (if ambiguous): {rel_A: '#7', rel_B: '#9'}. Blocked attributes: ['semi-firm']."`
  - Partial/degraded: `"Macro complete. Intersection empty — returning base variables for Solver. Base variables: {Goat_neighbors: '#25', cows_neighbors: '#38'}. Blocked attributes: []."`

GRACEFUL DEGRADATION (PARTIAL RESULTS REQUIREMENT):
You are strictly forbidden from returning a "blocked", "dead-end", or completely empty result IF you have successfully minted at least one neighbor Variable for any of the source nodes.
If your final `intersection()` calls result in an empty set or fail due to Knowledge Graph topology limits (i.e., the entities genuinely share no neighbors under any relation), you MUST degrade gracefully:
1. Do NOT return `status="blocked"` or an empty `answer_recommendation`.
2. Instead, return `status="done"` with a `pruned_observation` dict containing the individual base variables you successfully minted for each source node, e.g. `{"Goat_neighbors": "#25", "cows_neighbors": "#38"}`.
3. Set `answer_recommendation` to the partial-result visibility string (see VISIBILITY RULE above).
Your objective is to hand the stateful variables back to the environment so the downstream Solver agent has the information it needs to continue the task — even when a strict intersection is topologically impossible.
...
This ensures the downstream Solver agent reading the observation knows exactly what semantic data is stored inside that specific variable.

NO HETEROGENEOUS INTERSECTIONS (UNIVERSAL GRAPH RULE):
You cannot mathematically intersect base items directly with raw string literals. To apply attributes, first convert each literal into a KG variable using reverse-neighbor expansion (`get_relations` -> `get_neighbors`), then perform variable-to-variable intersections. Only if this conversion fails after bounded attempts may you return blocked attributes for Solver follow-up.
...
NO FAKE VARIABLES: If your internal string parsing fails (e.g., you cannot extract a valid variable reference from an API response), return `status='blocked'`. This rule applies to PARSING failures only — do NOT return `status='blocked'` for KG topology failures such as an empty intersection result. See GRACEFUL DEGRADATION above.
...
# RUN_PAYLOAD_OPTIONAL: ["task_text", "asked_for", "trace", "actions_spec", "run_id", "state_dir", "constraints", "output_contract", "draft_response", "candidate_output", "env_observation", "domain_hints", "target_concept"]
# INVOKE_EXAMPLE: {"args":[{"task_text":"...","asked_for":"...","trace":[],"actions_spec":{},"run_id":"r1","state_dir":"./state"}], "kwargs":{}}
# Example: {"args":[{"task_text":"...","asked_for":"...","trace":[],"actions_spec":{},"run_id":"r1","state_dir":"./state"}], "kwargs":{}}
...
def run(payload: dict) -> dict:
    ...
    try:
        ...
        computed_result = None  # E.g., "#3" or {"candidate_rel": "#3"} if ambiguity remains
        blocked_attributes = []
