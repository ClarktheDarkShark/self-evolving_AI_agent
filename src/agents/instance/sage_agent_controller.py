...
from src.sage.family_contracts import FamilyAttemptOutcome, get_family_contract
...
from src.sage.invoker import (
    build_sage_execution_failure_payload,
    execute_sage_code_with_result,
)
...
from src.sage.policy_contracts import (
...
from src.sage.plausibility_validator import (
    AnchorProbeResult,
    PlausibilityVerdict,
    VERDICT_REPAIRABLE_BAD_COUNT_SET,
    _count_answer_target_requires_explicit_semantics,
    build_repair_feedback,
    validate_sage_execution,
)
from src.sage.prompts import (
...
_TOOL_EVOLUTION_SKIP_MANUAL_FALLBACK_ENV = "SAGE_TOOL_EVOLUTION_SKIP_MANUAL_FALLBACK"
_RUNTIME_SINGLE_ANCHOR_BINDING_ENV = "SAGE_RUNTIME_SINGLE_ANCHOR_ANSWER_BINDING"
_RUNTIME_SINGLE_ANCHOR_TARGET_ENV = "SAGE_RUNTIME_SINGLE_ANCHOR_TARGET_SEMANTICS"
_RUNTIME_SINGLE_ANCHOR_LOW_TRUST_DYNAMIC_ENV = "SAGE_RUNTIME_SINGLE_ANCHOR_LOW_TRUST_DYNAMIC"
_RUNTIME_SINGLE_ANCHOR_PIVOT_DYNAMIC_ENV = "SAGE_RUNTIME_SINGLE_ANCHOR_PIVOT_DYNAMIC"
...
_RUNTIME_TERMINAL_LEAF_RELAXATION_ENV = "SAGE_RUNTIME_TERMINAL_LEAF_RELAXATION"


class SAGEAgentController(Agent):
    _SAGE_QUERY_TOOL_BASE_NAME = "sage_sparql_query_tool"
    _SAGE_QUERY_PLAN_MAX_ATTEMPTS = 2
    _SAGE_QUERY_CODE_MAX_ATTEMPTS = 3
    # Max repair iterations *after* the initial candidate (total attempts = 1 + this)
    _SAGE_REPAIR_MAX_ATTEMPTS = 3
    _SAGE_REPAIR_STAGNATION_REPEAT_LIMIT = 2
    ...
    def _inference(self, chat_history: ChatHistory) -> ChatHistoryItem:
        ...
        try:
            ...
            if "Macro result:" in (last_user_content or ""):
                ...
                bypass_response = self._maybe_bypass_manual_solver_for_trusted_macro(
                    last_user_content
                )
            ...
            grounding_card = self._build_sage_grounding_card(
                task_question,
            ...
            self._emit_generated_tools_event(
                {
                    "event": "sage_question_interpretation",
                    "mode": "sage",
            ...
            self._emit_generated_tools_event(
                {
                    ...
                    "target_concept_source": "sage_question",
                    "domain_hints_source": "sage_question",
                    "attribute_target_concept_source": "sage_question",
                    "has_tool_plan": False,
                    "target_concept": None,
            ...
            if action != "generate_tool":
                self._emit_generated_tools_event(
                    {
                        "event": "sage_orchestrator_action_overridden",
                        "mode": "sage",
            ...
            if action == "generate_tool":
                pipeline_stage = "plan"
                query_plan = self._generate_sage_query_plan(
                    task_question=task_question,
                ...
                if repair_failure_reason:
                    self._emit_repair_loop_rejection_finalization(
                        generated_tool_name=generated_tool_name,
                        query_plan=query_plan,
                        repair_loop_log=_repair_loop_log,
                ...
                query_artifacts = self._capture_sage_query_artifacts(
                    generated_tool_name=generated_tool_name,
                ...
                if not invocation_result.success or invocation_result.payload is None:
                    ...
                    failure_payload = build_sage_execution_failure_payload(
                        invocation_result,
                    ...
                    self._emit_generated_tools_event(
                        self._build_sage_query_execution_event(
                            generated_tool_name=generated_tool_name,
                            query_artifacts=query_artifacts,
                            invocation_result=invocation_result,
                            result_payload=None,
                    ...
                    self._emit_sage_invoker_failure_event(
                        generated_tool_name=generated_tool_name,
                        invocation_result=invocation_result,
                    )
                    pipeline_stage = "adapter"
                    materialization = self._adapt_sage_result(
                        task_question=task_question,
                    ...
                    self._emit_generated_tools_event(
                        {
                            "event": "sage_controller_execution_fallback_used",
                            "mode": "sage",
                            "tool_name": generated_tool_name,
                            "failure_kind": failure_kind,
                    ...
                    if not trust_contract.materialization_allowed:
                        self._maybe_record_family_policy_candidate(
                            ...
                            query_plan=query_plan,
                            failure_reason=(
                                "sage_materialization_contract_failed:"
                ...
                self._emit_generated_tools_event(
                    self._build_sage_query_execution_event(
                        ...
                        projected_variables=projected_variables,
                        result_kind=self._classify_sage_result_kind(result_dict),
                    )
                )
                raw_artifact = classify_execution_artifact(result_dict)
                solver_output = ""
                if self._should_skip_sage_solver_for_raw_artifact(raw_artifact):
                    self._emit_generated_tools_event(
                        {
                            "event": "sage_solver_skipped_for_structured_raw_result",
                            "mode": "sage",
                            "tool_name": generated_tool_name,
                            "artifact_type": raw_artifact.artifact_type,
                            "artifact_source": raw_artifact.source,
                            "binding_count": self._get_result_binding_count(result_dict),
                        }
                    )
                else:
            ...
            raise AgentUnknownException("sage_orchestrator_refused_generate_tool")
        except AgentUnknownException:
            raise
        except Exception as e:
            failure_reason = str(e).strip()
            if failure_reason.startswith("sage_query_plan_invalid:"):
                advisory_text = self._build_sage_plan_failure_advisory(
            ...
            self._emit_generated_tools_event(
                {
                    "event": "sage_adapter_failed" if pipeline_stage == "adapter" else "tool_generation_failed",
            ...
            raise AgentUnknownException(failure_reason) from e
    ...
    def _maybe_bypass_manual_solver_for_trusted_macro(
        self,
        macro_result_content: str,
    ) -> Optional[ChatHistoryItem]:
        if not self._tool_evolution_skip_manual_fallback_enabled():
            return None
        macro_pointer = self._extract_macro_pointer(macro_result_content)
        if not macro_pointer:
            return None
        response = ChatHistoryItem(
            role=Role.AGENT,
            content=f"Final Answer: {macro_pointer}",
        )
        self._tool_invoked_in_last_inference = "sage_trusted_macro_bypass"
        self._emit_generated_tools_event(
            {
                "event": "sage_trusted_macro_bypassed",
                "mode": "sage",
        ...
        self._emit_macro_solver_review(
            macro_result_content=macro_result_content,
            solver_response=response.content,
        )
        return response

    @staticmethod
    def _should_skip_sage_solver_for_raw_artifact(artifact: Any) -> bool:
        return bool(
            artifact is not None
            and getattr(artifact, "source", "") == "raw_execution"
            and getattr(artifact, "artifact_type", "")
            in {
                "entity_id",
                "entity_set",
                "count_scalar",
    ...
    def _bypass_manual_solver_after_tool_failure(
        ...
        *,
        ...
        self._tool_invoked_in_last_inference = "sage_tool_failure_bypass"
        self._emit_generated_tools_event(
            {
                "event": "sage_tool_failure_bypassed",
                "mode": "sage",
                "tool_name": generated_tool_name,
        ...
        raise AgentUnknownException(f"sage_tool_failure_bypassed:{cleaned_reason}")

    def _adapt_sage_result(
        self,
        *,
        task_question: str,
        raw_result: Any,
        ...
        adaptation = adapt_sage_result_to_benchmark(
            ...
            solver_output=solver_output,
            context=self._build_adapter_context(task_question, query_plan=query_plan),
    ...
    def _materialize_adapter_response(
        ...
        *,
        ...
        verdict: Optional[PlausibilityVerdict] = None,
    ) -> str:
        artifact_type = str(
            (materialization.diagnostics or {}).get("artifact_type") or ""
        ).strip()
        if materialization.needs_bridge:
            tool_name = self._ensure_bridge_tool(
                materialization.bridge_tool_name or SAGE_BENCHMARK_BRIDGE_TOOL_NAME
            )
            ...
            payload.setdefault(
                "sage_trusted_for_materialization",
                bool(
            ...
            payload.setdefault("sage_confidence", materialization.confidence)
            payload.setdefault(
                "sage_selected_query_variable",
                str(
            ...
            payload.setdefault(
                "sage_answer_cardinality_hint",
                self._infer_answer_cardinality_hint(task_question),
            )
            # ----------------------------------------------------------
            # Fix C — zero-count safety net
            # If a count_scalar of 0 is about to be sent as trusted-final
            # but the upstream plausibility checks did not add a strong
            # explicit-zero-acceptance reason, downgrade the trust flags
            # and attach a verification caveat.  This is a conservative
            # last-resort: Fixes A/B in the plausibility validator should
            # catch structural bad zeros before reaching here.
            # ----------------------------------------------------------
            if (
                str(payload.get("sage_artifact_type") or "").strip() == "count_scalar"
                and payload.get("sage_artifact_value") == 0
                and payload.get("sage_trusted_for_materialization")
            ):
                _zero_verdict_reasons = list(
                    getattr(verdict, "reasons", []) if verdict is not None else []
                )
                _strong_zero_accepted = any(
                    r in _zero_verdict_reasons
                    for r in (
                        "accepted_exact_grounded_zero_joined_count",
                        "accepted_pinned_semantic_dynamic_count",
                    )
                )
                if not _strong_zero_accepted:
                    payload["sage_trusted_for_materialization"] = False
                    payload["sage_solves_task"] = False
                    payload["sage_tool_status"] = "partial"
                    payload["sage_repair_caveat"] = (
                        "Count result is zero but no strong structural justification "
                        "for an exact zero was established. Verify the query traverses "
                        "the intended anchor-to-count-set path before accepting this "
                        "as a final answer."
                    )
            ...
            self._pending_macro_runs[run_id] = {
                ...
                "artifact_type": payload.get("sage_artifact_type"),
            }
            ...
            self._emit_generated_tools_event(
                {
                    "event": "sage_macro_invoke_requested",
                    "tool_name": tool_name,
                    "run_id": run_id,
                    "state_dir": payload.get("state_dir"),
                    "artifact_type": payload.get("sage_artifact_type"),
                    "artifact_source": payload.get("sage_artifact_source"),
        ...
        if artifact_type == "unresolved":
            raise AgentUnknownException("sage_adapter_unresolved_artifact")
        raise AgentUnknownException("sage_adapter_empty_materialization")
    ...
    def _build_attempt_decision_record(
        ...
        *,
        ...
        family_contract = get_family_contract(selected_family or query_shape)
        ...
        return AttemptDecisionRecord(
            ...
            family_contract=(
                family_contract.as_dict() if family_contract is not None else {}
            ),
        )
    ...
    def _maybe_record_family_policy_candidate(
        ...
        *,
        ...
        try:
            ...
            candidate = store.create_candidate_update(
                ...
                trigger_context={
                    ...
                    "failure_class": failure_class,
                    "stop_reason": str(
                        (repair_loop_log or {}).get("stop_reason") or ""
                    ).strip(),
                    "primary_failure_kind": self._classify_family_attempt_failure_kind(
                        loop_log=repair_loop_log or {},
                        trust_contract=(
                            trust_contract
                            or build_trust_contract_evaluation(
                                plan_consistency_passed=True,
                                execution_shape_passed=True,
                                plausibility_validation_passed=False,
                                adapter_safety_passed=True,
                                extra_denial_reasons=(),
                            )
                        ),
                        tool_result_semantics=None,
                    ),
                    "family_contract": (
                        get_family_contract(selected_family).as_dict()
                        if get_family_contract(selected_family) is not None
                        else {}
                    ),
                },
            )
            ...
            self._emit_generated_tools_event(
                {
                    "event": "sage_family_policy_candidate_failed",
                    "mode": "sage",
                    "tool_name": generated_tool_name,
                    "family_name": selected_family,
                    "error": str(exc),
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_family_policy_candidate_created",
                "mode": "sage",
                "tool_name": generated_tool_name,
                "family_name": selected_family,
    ...
    def _evaluate_materialization_trust_contract(
        ...
        *,
        ...
        plan_validation_errors = self._validate_sage_query_candidate(
    ...
    def _emit_final_attempt_decision(
        ...
        *,
        ...
        typed_outcome = self._build_family_attempt_outcome(
            query_plan=query_plan,
            loop_log=loop_log,
            trust_contract=trust_contract,
            tool_result_semantics=tool_result_semantics,
        ...
        final_payload.update(
            {
                ...
                "typed_outcome": typed_outcome.as_dict(),
                "stop_reason": typed_outcome.stop_reason,
                "primary_failure_kind": typed_outcome.primary_failure_kind,
                "completion_state": typed_outcome.completion_state,
            }
        )
    ...
    def _resolve_family_attempt_stop_reason(
        self,
        *,
        loop_log: Mapping[str, Any],
        trust_contract: TrustContractEvaluation,
        tool_result_semantics: Mapping[str, Any] | None,
    ) -> str:
        explicit_stop_reason = str(loop_log.get("stop_reason") or "").strip()
        if explicit_stop_reason:
            return explicit_stop_reason
        failure_reason = str(
            (tool_result_semantics or {}).get("tool_result_failure_reason") or ""
        ).strip()
        verdict = str(
            loop_log.get("last_verdict") or loop_log.get("final_verdict") or ""
        ).strip()
        last_reasons = [
            str(item or "").strip()
            for item in (loop_log.get("last_reasons") or [])
            if str(item or "").strip()
        ]
        denial_reasons = {
            str(item or "").strip()
            for item in (trust_contract.denial_reasons or ())
            if str(item or "").strip()
        }
        if trust_contract.materialization_allowed:
            return "accepted_completion"
        if "plan_consistency_failed" in denial_reasons:
            return "validator_blocked"
        if "execution_shape_mismatch" in denial_reasons:
            return "solver_handoff_failure"
        if "adapter_safety_failed" in denial_reasons:
            return "solver_handoff_failure"
        if any(
            fragment in failure_reason
            for fragment in {
                "endpoint_timeout",
                "timed_out",
            }
        ...
        if verdict in {
            ...
            reason.startswith(("anchor_not_found", "anchor_path_empty"))
            or "weak_grounding" in reason
            for reason in last_reasons
        ):
            return "grounding_miss"
        if verdict == "rejected_dangerous_overreach" or any(
            reason.startswith("dangerous_overreach")
            for reason in last_reasons
        ):
            return "validator_blocked"
        if verdict.startswith("rejected_") or any(
            reason.startswith(
                (
                    "single_use_helper_variable:",
                    "projected_entity_not_structurally_bound:",
                    "ungrounded_type_set_variable:",
                )
            )
            for reason in list(denial_reasons) + last_reasons
        ):
            return "validator_blocked"
        return "switch_family"

    def _classify_family_attempt_failure_kind(
        self,
        *,
        loop_log: Mapping[str, Any],
        trust_contract: TrustContractEvaluation,
        ...
        stop_reason = self._resolve_family_attempt_stop_reason(
            loop_log=loop_log,
            trust_contract=trust_contract,
            tool_result_semantics=tool_result_semantics,
        )
        ...
        if stop_reason == "accepted_completion":
            return "accepted_completion"
        if stop_reason == "endpoint_timeout":
            return "timeout"
        if stop_reason == "infrastructure_failure":
            return "infrastructure_failure"
        if stop_reason == "grounding_miss":
            return "grounding_miss"
        if stop_reason == "solver_handoff_failure":
            return "solver_handoff_failure"
        verdict = str(
            loop_log.get("last_verdict") or loop_log.get("final_verdict") or ""
        ).strip()
        ...
        if verdict == "rejected_dangerous_overreach" or any(
            ...
            return "semantic_family_failure"
        if "plan_consistency_failed" in denial_reasons:
            return "plan_inconsistency"
        if "execution_shape_mismatch" in denial_reasons:
            return "execution_shape_failure"
        if stop_reason == "validator_blocked":
            return "validator_rejection"
        return "semantic_family_failure"

    def _build_family_attempt_outcome(
        self,
        *,
        query_plan: Mapping[str, Any],
        loop_log: Mapping[str, Any],
        ...
        query_shape = str(query_plan.get("query_shape") or "").strip()
        family_contract = get_family_contract(query_shape)
        stop_reason = self._resolve_family_attempt_stop_reason(
            loop_log=loop_log,
            trust_contract=trust_contract,
            tool_result_semantics=tool_result_semantics,
        ...
        best_candidate = dict(loop_log.get("best_executing_candidate") or {})
        grounding_summary = {
            "anchor_probe_count": len(loop_log.get("last_anchor_probes") or []),
            "resolved_anchor_count": sum(
                1
                for item in (loop_log.get("last_anchor_probes") or [])
                if bool(item.get("resolved_entity_id"))
            ),
            "all_anchors_found": all(
                bool(item.get("found"))
                for item in (loop_log.get("last_anchor_probes") or [])
            )
            if loop_log.get("last_anchor_probes")
            else None,
        }
        return FamilyAttemptOutcome(
            ...
            execution_success=bool(best_candidate),
            binding_count=best_candidate.get("binding_count"),
            scalar_count=best_candidate.get("scalar_count"),
            repair_attempt_count=int(loop_log.get("total_attempts") or 0),
            repair_budget_exhausted=(
                str(stop_reason).strip() == "repair_budget_exhausted"
            ),
            stagnation_detected=bool(loop_log.get("stagnation_detected")),
            candidate_score=best_candidate.get("score"),
            grounding_summary=grounding_summary,
            trust_contract=trust_contract.as_dict(),
            family_contract=family_contract.as_dict() if family_contract is not None else {},
        )
    ...
    def _emit_sage_invoker_failure_event(
        self,
        *,
        generated_tool_name: Optional[str],
        ...
        event_name = (
            ...
            else "sage_invoker_transport_error"
            if failure_kind in {"transport_error", "endpoint_timeout"}
            else "sage_invoker_execution_error"
        )
        self._emit_generated_tools_event(
            {
                "event": event_name,
    ...
    def _build_query_tool_name(self, task_question: str) -> str:
        ...
        return f"{self._SAGE_QUERY_TOOL_BASE_NAME}_{sample_index}_{question_hash}"
    ...
    def _get_generated_tools_log_path(self) -> Path:
        output_dir = Path(
            os.environ.get("LIFELONG_OUTPUT_DIR", "outputs/sage_runtime")
        )
    ...
    def _classify_sage_result_kind(self, result_dict: Mapping[str, Any]) -> str:
    ...
    def _generate_sage_query_plan(
        ...
        *,
        ...
        for attempt in range(1, self._SAGE_QUERY_PLAN_MAX_ATTEMPTS + 1):
            ...
            try:
                query_plan = self._parse_sage_query_plan(plan_raw)
                ...
                if answer_target_phrase and not str(
                    ...
                    query_plan = self._normalize_sage_query_plan(
                        {
                            **dict(query_plan),
                ...
                self._emit_generated_tools_event(
                    {
                        "event": "sage_query_plan_rejected",
                        "mode": "sage",
                        "tool_name": generated_tool_name,
                        "attempt": attempt,
                        "error": last_error,
                ...
                continue
            plan_artifact_path = self._capture_sage_query_plan_artifact(
                generated_tool_name=generated_tool_name,
                query_plan=query_plan,
            )
            self._emit_generated_tools_event(
    ...
    def _parse_sage_query_plan(self, raw_output: str) -> dict[str, Any]:
        ...
        return self._normalize_sage_query_plan(parsed)

    def _normalize_sage_query_plan(self, plan: Mapping[str, Any]) -> dict[str, Any]:
        ...
        (
            shared_answer_variable,
            projection,
        ) = self._normalize_entity_projection_to_structural_candidate_set(
            answer_mode=answer_mode,
            query_shape=query_shape,
            relation_paths=relation_paths,
            shared_answer_variable=shared_answer_variable,
    ...
    def _ensure_repair_loop_accepted(
        ...
        *,
        ...
        stop_reason = self._derive_repair_loop_rejection_stop_reason(
            repair_loop_log=repair_loop_log
        )
        self._emit_generated_tools_event(
            {
                ...
                "stagnation_detected": bool(
                    repair_loop_log.get("stagnation_detected")
                ),
            }
        )
        self._maybe_record_family_policy_candidate(
            ...
            failure_reason=f"sage_query_not_accepted:{last_verdict}",
            repair_loop_log=repair_loop_log,
        )
        return f"sage_query_not_accepted:{last_verdict}"

    def _derive_repair_loop_rejection_stop_reason(
        self,
        *,
        repair_loop_log: Mapping[str, Any],
    ) -> str:
        explicit = str(repair_loop_log.get("stop_reason") or "").strip()
        if explicit:
            return explicit
        last_verdict = str(
            repair_loop_log.get("last_verdict")
            or repair_loop_log.get("final_verdict")
            or "no_accepted_candidate"
        ).strip()
        if last_verdict in {
            "repairable_anchor_not_found",
            "repairable_anchor_path_empty",
            ...
            return "grounding_miss"
        if last_verdict.startswith("rejected_"):
            return "validator_blocked"
        return "repair_budget_exhausted"

    def _emit_repair_loop_rejection_finalization(
        self,
        *,
        generated_tool_name: str,
        query_plan: Mapping[str, Any],
        ...
        effective_loop_log = dict(repair_loop_log)
        effective_loop_log["stop_reason"] = self._derive_repair_loop_rejection_stop_reason(
            repair_loop_log=repair_loop_log
        )
        last_verdict = str(
            effective_loop_log.get("last_verdict")
            or effective_loop_log.get("final_verdict")
            or ""
        ).strip()
        last_reasons = [
            str(item or "").strip()
            for item in (effective_loop_log.get("last_reasons") or [])
            if str(item or "").strip()
        ]
        dangerous_overreach = last_verdict == "rejected_dangerous_overreach" or any(
            reason.startswith("dangerous_overreach")
            for reason in last_reasons
        )
        trust_contract = build_trust_contract_evaluation(
            plan_consistency_passed=effective_loop_log["stop_reason"] != "validator_blocked",
            execution_shape_passed=True,
            plausibility_validation_passed=False,
            adapter_safety_passed=True,
            extra_denial_reasons=("repair_loop_rejected",),
            dangerous_overreach_reasons=tuple(last_reasons) if dangerous_overreach else (),
        )
        materialization = BenchmarkMaterialization(
            materialization_type="repair_loop_rejected",
            needs_bridge=False,
            bridge_action=None,
            ...
            diagnostics={
                "artifact_type": "repair_loop_rejected",
                "artifact_source": "sage_repair_loop",
                "family_name": str(query_plan.get("query_shape") or "").strip(),
                "query_shape": str(query_plan.get("query_shape") or "").strip(),
            },
            confidence=0.0,
            determinism_level="none",
            tool_status="failed",
            semantic_description=repair_failure_reason,
            solves_task=False,
    ...
    def _build_best_executing_candidate_metadata(
        ...
        *,
        ...
        plan_validation_errors = list(
            self._validate_sage_query_candidate(
                raw_output=generated_code,
                generated_code=generated_code,
                query_text=query_text,
                query_texts=self._extract_sparql_query_texts(generated_code),
    ...
    def _normalize_entity_projection_to_structural_candidate_set(
        self,
        *,
        answer_mode: str,
        query_shape: str,
        ...
        if str(answer_mode or "").strip().lower() != "entity":
            return shared_answer_variable, list(projection or [])
        if str(query_shape or "").strip().lower() not in {
            "single_anchor_lookup",
            "multi_anchor_intersection",
        }:
            return shared_answer_variable, list(projection or [])
        ...
        if not normalized_projection:
            return shared_answer_variable, normalized_projection

        candidate_token = self._normalize_variable_token(candidate_set_variable)
        projected_token = self._normalize_variable_token(normalized_projection[0])
        shared_answer_token = self._normalize_variable_token(shared_answer_variable)
        if (
            projected_token in {"", candidate_token}
            or projected_token == shared_answer_token == candidate_token
        ):
            return shared_answer_variable, normalized_projection

        structural_tokens: set[str] = set()
        answer_role_tokens: set[str] = set()
        for relation_path in relation_paths:
            if not isinstance(relation_path, Mapping):
                continue
            for endpoint_key, role_key in (("from", "from_role"), ("to", "to_role")):
                endpoint_token = self._normalize_variable_token(
                    relation_path.get(endpoint_key)
                )
                role_token = self._normalize_relation_role(relation_path.get(role_key))
                if endpoint_token:
                    structural_tokens.add(endpoint_token)
                if role_token in {"answer", "shared_answer"} and endpoint_token:
                    answer_role_tokens.add(endpoint_token)

        if candidate_token not in structural_tokens:
            return shared_answer_variable, normalized_projection
        if projected_token in structural_tokens:
            return shared_answer_variable, normalized_projection
        if shared_answer_token and shared_answer_token in answer_role_tokens:
            return shared_answer_variable, normalized_projection
        if answer_role_tokens:
            return shared_answer_variable, normalized_projection

        if candidate_token and candidate_token in structural_tokens:
            return candidate_set_variable, [candidate_set_variable]
        return candidate_set_variable, [candidate_set_variable]
    ...
    def _runtime_single_anchor_binding_enabled(self) -> bool:
        return os.getenv(_RUNTIME_SINGLE_ANCHOR_BINDING_ENV) == "1"

    def _runtime_single_anchor_target_semantics_enabled(self) -> bool:
        return os.getenv(_RUNTIME_SINGLE_ANCHOR_TARGET_ENV) == "1"

    def _runtime_single_anchor_low_trust_dynamic_enabled(self) -> bool:
        return os.getenv(_RUNTIME_SINGLE_ANCHOR_LOW_TRUST_DYNAMIC_ENV) == "1"

    def _runtime_single_anchor_pivot_dynamic_enabled(self) -> bool:
        return os.getenv(_RUNTIME_SINGLE_ANCHOR_PIVOT_DYNAMIC_ENV) == "1"

    def _runtime_single_anchor_chain_low_trust_dynamic_enabled(self) -> bool:
        return os.getenv("SAGE_RUNTIME_SINGLE_ANCHOR_CHAIN_LOW_TRUST_DYNAMIC") == "1"

    def _runtime_preserve_chain_query_shape_on_refresh_enabled(self) -> bool:
        return os.getenv("SAGE_RUNTIME_PRESERVE_CHAIN_QUERY_SHAPE_ON_REFRESH") == "1"

    def _runtime_joined_count_target_boundary_enabled(self) -> bool:
        return os.getenv(_RUNTIME_JOINED_COUNT_TARGET_ENV) == "1"

    def _runtime_dynamic_probe_fb_filter_enabled(self) -> bool:
        return os.getenv(_RUNTIME_DYNAMIC_PROBE_FB_FILTER_ENV, "0").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def _runtime_dynamic_probe_allow_base_enabled(self) -> bool:
        return os.getenv(_RUNTIME_DYNAMIC_PROBE_ALLOW_BASE_ENV, "0").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def _runtime_terminal_leaf_relaxation_enabled(self) -> bool:
        return os.getenv(_RUNTIME_TERMINAL_LEAF_RELAXATION_ENV, "1").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def _single_anchor_answer_binding_errors(
        self,
        *,
        query_plan: Mapping[str, Any],
    ) -> list[str]:
        if str(query_plan.get("query_shape") or "").strip() != "single_anchor_lookup":
            return []
        relation_paths = [
            relation_path
        ...
        if len(relation_paths) != 1:
            return ["family_policy_single_anchor_requires_direct_path"]
        relation_path = relation_paths[0]
        from_role = self._normalize_relation_role(relation_path.get("from_role"))
        to_role = self._normalize_relation_role(relation_path.get("to_role"))
        if {from_role, to_role} == {"anchor", "answer"}:
            return []
        return ["family_policy_single_anchor_requires_direct_path"]

    def _single_anchor_answer_target_semantics_errors(
        self,
        *,
        query_plan: Mapping[str, Any],
        relation_grounding: Sequence[Mapping[str, str]],
        ...
        if not answer_target_phrase or not self._count_answer_target_requires_explicit_semantics(
            answer_target_phrase
        ):
            return []
        relation_paths = [
        ...
        if not relation_paths:
            return []
        planned_semantics_preserved = (
            self._count_answer_target_head_is_encoded_in_relation(
                answer_target_phrase=answer_target_phrase,
                relation_candidates=relation_paths,
            )
            or self._answer_target_qualifier_is_encoded_in_answer_relation(
                answer_target_phrase=answer_target_phrase,
                relation_candidates=relation_paths,
            )
            or self._answer_target_is_relation_encoded_type_projection(
                answer_target_phrase=answer_target_phrase,
                relation_candidates=relation_paths,
            )
        )
        if planned_semantics_preserved:
            return []
        grounded_semantics_available = (
            self._count_answer_target_head_is_encoded_in_relation(
                answer_target_phrase=answer_target_phrase,
                relation_candidates=relation_grounding,
            )
            or self._answer_target_qualifier_is_encoded_in_answer_relation(
                answer_target_phrase=answer_target_phrase,
                relation_candidates=relation_grounding,
        ...
        if not grounded_semantics_available:
            return []
        if any(
            str(relation_path.get("grounding_source") or "").strip() == "exploratory"
            for relation_path in relation_paths
        ):
            return [
                "family_policy_single_anchor_exploratory_answer_target_drift:"
                + answer_target_phrase
            ]
        return [
            "family_policy_single_anchor_answer_target_not_preserved:"
            + answer_target_phrase
        ]

    def _single_anchor_low_trust_dynamic_errors(
        self,
        *,
        query_plan: Mapping[str, Any],
    ) -> list[str]:
        query_shape = str(query_plan.get("query_shape") or "").strip()
        if query_shape not in {"single_anchor_lookup", "single_anchor_chain_lookup"}:
            return []
        if (
            query_shape == "single_anchor_chain_lookup"
            and not self._runtime_single_anchor_chain_low_trust_dynamic_enabled()
        ):
            return []
        if (
            query_shape == "single_anchor_lookup"
            and not bool(query_plan.get("allow_exploratory_predicates"))
        ):
            return []
        relation_paths = [
            relation_path
        ...
        if len(relation_paths) != 1:
            return []
        relation_path = relation_paths[0]
        if str(relation_path.get("grounding_source") or "").strip() != "dynamic_probe":
            return []
        from_role = self._normalize_relation_role(relation_path.get("from_role"))
        to_role = self._normalize_relation_role(relation_path.get("to_role"))
        if query_shape == "single_anchor_lookup" and {from_role, to_role} != {"anchor", "answer"}:
            return []
        if query_shape == "single_anchor_chain_lookup" and (
            from_role != "anchor" or to_role != "candidate_set"
        ):
            return []

        alias_repaired = any(
            "live probe repair selected alias"
            in str(item.get("reason") or "").strip().lower()
            for item in (query_plan.get("normalized_aliases") or [])
            if isinstance(item, Mapping)
        )
        alias_shifted = any(
            str(item.get("role") or "").strip() == "anchor"
            and str(item.get("surface") or "").strip()
            and str(item.get("chosen_alias") or "").strip()
            and str(item.get("resolved_entity_id") or "").strip()
            and str(item.get("surface") or "").strip()
            != str(item.get("chosen_alias") or "").strip()
            for item in (query_plan.get("anchored_entities") or [])
            if isinstance(item, Mapping)
        )
        if not (alias_repaired and alias_shifted):
            return []
        if query_shape == "single_anchor_chain_lookup":
            candidate_token = self._normalize_variable_token(
                query_plan.get("candidate_set_variable")
            )
            answer_token = self._normalize_variable_token(
                query_plan.get("shared_answer_variable")
            )
            if not candidate_token or candidate_token == answer_token:
                return []
        return ["family_policy_single_anchor_low_trust_dynamic_alias_repair"]

    def _query_plan_has_single_anchor_chain_structure(
        self,
        *,
        query_plan: Mapping[str, Any],
    ) -> bool:
        ...
        return (
            ...
            and candidate_token != answer_token
        )

    def _preserve_chain_query_shape_on_refresh(
        self,
        *,
        previous_query_plan: Mapping[str, Any],
        refreshed_query_plan: Mapping[str, Any],
    ) -> Mapping[str, Any]:
        if not self._runtime_preserve_chain_query_shape_on_refresh_enabled():
            return refreshed_query_plan
        previous_shape = str(previous_query_plan.get("query_shape") or "").strip()
        refreshed_shape = str(refreshed_query_plan.get("query_shape") or "").strip()
        if previous_shape != "single_anchor_chain_lookup":
            return refreshed_query_plan
        if refreshed_shape == "single_anchor_chain_lookup":
            return refreshed_query_plan
        if not self._query_plan_has_single_anchor_chain_structure(
            query_plan=refreshed_query_plan
        ):
            return refreshed_query_plan
        preserved = dict(refreshed_query_plan)
        preserved["query_shape"] = "single_anchor_chain_lookup"
        return preserved

    def _joined_count_target_boundary_errors(
        self,
        *,
        query_plan: Mapping[str, Any],
        relation_grounding: Sequence[Mapping[str, str]],
    ) -> list[str]:
        if str(query_plan.get("query_shape") or "").strip() != "count_over_joined_set":
            return []
        relation_paths = [
            relation_path
            for relation_path in (query_plan.get("relation_paths") or [])
        ...
        if not relation_paths:
            return []
        count_token = self._normalize_variable_token(query_plan.get("count_set_variable"))
        candidate_token = self._normalize_variable_token(
            query_plan.get("candidate_set_variable")
        )
        shared_answer_token = self._normalize_variable_token(
            query_plan.get("shared_answer_variable")
        )
        count_bound = False
        count_connected = candidate_token == count_token and bool(count_token)
        for relation_path in relation_paths:
            from_token = self._normalize_variable_token(relation_path.get("from"))
            to_token = self._normalize_variable_token(relation_path.get("to"))
            from_role = self._normalize_relation_role(relation_path.get("from_role"))
            to_role = self._normalize_relation_role(relation_path.get("to_role"))
            if count_token and (
                count_token in {from_token, to_token}
                or "count_set" in {from_role, to_role}
            ):
                count_bound = True
            if count_token and candidate_token and count_token != candidate_token:
                if (
                    {count_token, candidate_token} == {from_token, to_token}
                    or {"count_set", "candidate_set"} == {from_role, to_role}
                ):
                    count_connected = True
            if count_token and shared_answer_token and count_token != shared_answer_token:
                if (
                    {count_token, shared_answer_token} == {from_token, to_token}
                    or {from_role, to_role} & {"count_set"}
                    and {from_role, to_role} & {"answer", "shared_answer"}
                ):
                    count_connected = True
        errors: list[str] = []
        if count_token and not count_bound:
            errors.append(
                "count_query_counts_wrong_variable:count_set_not_structurally_bound"
            )
        if count_token and candidate_token and count_token != candidate_token and not count_connected:
            errors.append(
                "count_answer_target_unenforced:count_target_not_connected_to_candidate_boundary"
            )
        answer_target_phrase = str(query_plan.get("answer_target_phrase") or "").strip()
        if (
            answer_target_phrase
            ...
            errors.append(
                "family_policy_count_answer_target_not_preserved:"
                + answer_target_phrase
            )
        return errors

    def _active_family_runtime_policy_errors(
        self,
        *,
        query_plan: Mapping[str, Any],
        relation_grounding: Sequence[Mapping[str, str]] | None = None,
    ) -> list[str]:
        errors: list[str] = []
        locked_family = compare_locked_family_name()
        query_shape = str(query_plan.get("query_shape") or "").strip()
        if locked_family and query_shape and query_shape != locked_family:
            errors.append(
        ...
        active_family_bundle = get_reusable_family_policy_bundle(query_shape)
        if active_family_bundle is None:
            return errors

        validator_expectations = set(active_family_bundle.validator_expectations)
        scaffold_signature = self._build_scaffold_signature(query_plan)
        if (
            scaffold_signature
            and scaffold_signature in set(active_family_bundle.blocked_scaffold_signatures)
        ):
            errors.append("family_policy_blocked_scaffold_signature:" + scaffold_signature)

        forbidden_relation_families = set(active_family_bundle.forbidden_relation_families)
        if forbidden_relation_families:
            used_relation_names = set(self._relation_names_from_plan(query_plan))
            for relation_name in sorted(used_relation_names & forbidden_relation_families):
                errors.append("family_policy_forbidden_relation_family:" + relation_name)

        if (
            self._runtime_single_anchor_binding_enabled()
            and validator_expectations
            & {
                "require_direct_single_anchor_answer_path",
                "verify_projected_entity_matches_question_target",
            }
        ):
            errors.extend(self._single_anchor_answer_binding_errors(query_plan=query_plan))

        if (
            relation_grounding is not None
            and self._runtime_single_anchor_target_semantics_enabled()
            and validator_expectations
            & {
            ...
            errors.extend(
                self._single_anchor_answer_target_semantics_errors(
                    query_plan=query_plan,
                    relation_grounding=relation_grounding,
                )
            )

        if self._runtime_single_anchor_low_trust_dynamic_enabled():
            errors.extend(
                self._single_anchor_low_trust_dynamic_errors(query_plan=query_plan)
            )

        if (
            relation_grounding is not None
            and self._runtime_joined_count_target_boundary_enabled()
            ...
            errors.extend(
                self._joined_count_target_boundary_errors(
                    query_plan=query_plan,
                    relation_grounding=relation_grounding,
                )
            )
        ...
        *,
        ...
        if not relation_grounding:
            if not allow_exploratory and relation_paths:
                self._emit_generated_tools_event(
                    {
                        "event": "sage_plan_rejected_no_grounding",
                        "mode": "sage",
        ...
        errors: list[str] = list(
            self._active_family_runtime_policy_errors(
                query_plan=query_plan,
                relation_grounding=relation_grounding,
            )
        )
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_grounding_validation",
                "mode": "sage",
                "query_shape": query_plan.get("query_shape"),
    ...
    def _build_grounded_relation_candidates(
        ...
        if any(kw in lower_text for kw in ("song", "music", "recording", "track", "release", "album", "musical")):
            ...
            candidates.extend(music_candidates)
    ...
    def _capture_sage_query_plan_artifact(
        self,
        *,
        generated_tool_name: str,
        query_plan: Mapping[str, Any],
    ) -> Path:
        artifact_dir = self._get_sage_query_artifact_dir()
    ...
    def _generate_validated_sage_candidate(
        self,
        *,
        task_question: str,
        grounding_card: str,
        ...
        if reusable_selection is not None:
            self._emit_generated_tools_event(
                {
                    "event": "sage_reusable_tool_selected",
                    "mode": "sage",
                    "tool_name": generated_tool_name,
            ...
            try:
                reusable_query_plan = self._canonicalize_reusable_query_plan_for_render(
                    query_plan=query_plan,
                    relation_grounding=relation_grounding,
                )
                generated_output = render_reusable_tool(
                    query_plan=reusable_query_plan,
                ...
                reusable_errors = self._validate_sage_query_candidate(
                ...
                if reusable_errors:
                    ...
                    self._emit_generated_tools_event(
                        {
                            "event": "sage_reusable_tool_fallback",
                            "mode": "sage",
                            "tool_name": generated_tool_name,
                            "family_name": reusable_selection.family_name,
                            "reason": last_error,
                    ...
                    self._emit_generated_tools_event(
                        {
                            "event": "sage_query_candidate_accepted",
                            "mode": "sage",
                            "tool_name": generated_tool_name,
                            "attempt": 0,
        ...
        for attempt in range(1, self._SAGE_QUERY_CODE_MAX_ATTEMPTS + 1):
            ...
            query_text = query_texts[0] if query_texts else ""
            validation_errors = self._validate_sage_query_candidate(
                raw_output=generated_output,
                generated_code=generated_code,
                query_text=query_text,
                query_texts=query_texts,
        ...
        raise ValueError(
            f"sage_query_candidate_invalid:{last_error or 'validation_failed'}"
        )

    def _canonicalize_reusable_query_plan_for_render(
        self,
        *,
        query_plan: Mapping[str, Any],
        relation_grounding: Sequence[Mapping[str, Any]] = (),
    ) -> dict[str, Any]:
        canonicalized_plan = self._normalize_sage_query_plan(copy.deepcopy(dict(query_plan)))
        if os.getenv(
            _RUNTIME_REUSABLE_SWAP_RENDER_CANONICALIZATION_ENV,
            "1",
        ).strip().lower() in {
        ...
        relation_paths = [
            dict(path)
            for path in (canonicalized_plan.get("relation_paths") or [])
            if isinstance(path, Mapping)
        ]
        if not relation_paths:
            return canonicalized_plan
        ...
        anchored_entities = [
            dict(item)
            for item in (canonicalized_plan.get("anchored_entities") or [])
            if isinstance(item, Mapping)
        ]
        anchor_count = len(
            [
        ...
        rewrite_events: list[dict[str, Any]] = []
        rewritten_paths: list[dict[str, Any]] = []
        for relation_path in relation_paths:
            rewritten_path = dict(relation_path)
            relation_name = str(rewritten_path.get("relation") or "").strip()
            grounding_source = str(
                rewritten_path.get("grounding_source") or ""
            ).strip().lower()
            if not relation_name or grounding_source == "exploratory":
                rewritten_paths.append(rewritten_path)
                continue
            candidates = [
                dict(candidate)
                for candidate in relation_grounding
                if isinstance(candidate, Mapping)
                and str(candidate.get("relation") or "").strip() == relation_name
            ]
            if not candidates:
                rewritten_paths.append(rewritten_path)
                continue
            for candidate in candidates:
                matched, reason = self._relation_contract_match_details(
                    planned_path=rewritten_path,
                    grounded_candidate=candidate,
                    query_shape=str(canonicalized_plan.get("query_shape") or ""),
                    anchor_count=anchor_count,
                    anchored_entities=anchored_entities,
                    query_plan=canonicalized_plan,
                )
                if not matched:
                    continue
                if reason != "accepted_role_swap_match":
                    break
                rewritten_path = {
                    **rewritten_path,
                    "from": str(relation_path.get("to") or "").strip(),
                    "to": str(relation_path.get("from") or "").strip(),
                    "from_role": str(relation_path.get("to_role") or "").strip(),
                    "to_role": str(relation_path.get("from_role") or "").strip(),
                }
                rewrite_events.append(
                    {
                        "relation": relation_name,
                        "direction": rewritten_path.get("direction"),
                        "reason": reason,
                        "original_from": relation_path.get("from"),
                        "original_to": relation_path.get("to"),
                        "render_from": rewritten_path.get("from"),
                        "render_to": rewritten_path.get("to"),
                        "original_from_role": relation_path.get("from_role"),
                        "original_to_role": relation_path.get("to_role"),
                        "render_from_role": rewritten_path.get("from_role"),
                        "render_to_role": rewritten_path.get("to_role"),
                    }
                )
                break
            rewritten_paths.append(rewritten_path)
        canonicalized_plan["relation_paths"] = rewritten_paths
        if rewrite_events:
            self._emit_generated_tools_event(
                {
                    "event": "sage_reusable_render_path_canonicalized",
                    "mode": "sage",
                    "query_shape": canonicalized_plan.get("query_shape"),
                    "rewrites": rewrite_events,
                }
            )
        return canonicalized_plan
    ...
    def _retry_execution_with_resolved_anchor_ids(
        ...
        *,
        ...
        retried_result = execute_sage_code_with_result(rewritten_code)
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_same_attempt_anchor_entity_retry",
                "mode": "sage",
    ...
    def _run_sage_repair_loop(
        ...
        *,
        ...
        """Bounded SAGE repair loop: generate → execute → validate → repair.

        Produces at most ``2 + _SAGE_REPAIR_MAX_ATTEMPTS`` full attempts.
        ...
        Logs one ``sage_repair_loop_attempt`` event per iteration so the loop
        is fully auditable from the generated_tools log.
        """
        max_total: int = self._SAGE_REPAIR_MAX_ATTEMPTS + 2  # initial + repairs + one structural rewrite retry
        ...
        for attempt in range(1, max_total + 1):
            ...
            loop_log["elapsed_seconds"] = round(
                float(time.monotonic() - loop_started_at),
                3,
            )

            # ---- Code generation ----------------------------------------
            try:
                generated_code, generation_metadata = self._generate_validated_sage_candidate(
                    task_question=task_question,
                ...
                self._emit_generated_tools_event(
                    {
                        "event": "sage_repair_loop_codegen_failed",
                        "mode": "sage",
                        "tool_name": generated_tool_name,
                        "attempt": attempt,
                        "is_repair": is_repair,
            ...
            # ---- Execution ----------------------------------------------
            invocation_result = execute_sage_code_with_result(generated_code)
            ...
            # ---- Plausibility check -------------------------------------
            verdict: PlausibilityVerdict = validate_sage_execution(
                query_plan=working_query_plan,
                query_text=query_text,
                result_dict=result_dict,
            ...
            if (
                ...
                self._emit_generated_tools_event(
                    {
                        "event": "sage_repair_feedback_constraint_triggered",
                        "mode": "sage",
                        "tool_name": generated_tool_name,
                        "attempt": attempt,
            ...
            self._emit_generated_tools_event(
                {
                    "event": "sage_repair_loop_attempt",
                    "mode": "sage",
                    "tool_name": generated_tool_name,
                    "attempt": attempt,
                    "is_repair": is_repair,
            ...
            if verdict.is_accepted:
                ...
                loop_log["stop_reason"] = "accepted_completion"
                self._emit_generated_tools_event(
                    {
                        "event": "sage_repair_loop_accepted",
                        "mode": "sage",
                        "tool_name": generated_tool_name,
                        "attempt": attempt,
            ...
            if signature_repeat_count >= self._SAGE_REPAIR_STAGNATION_REPEAT_LIMIT:
                loop_log["final_verdict"] = verdict.verdict
                loop_log["last_reasons"] = list(verdict.reasons) + [
                    "repair_loop_stalled"
                ]
                loop_log["stop_reason"] = "repeated_stagnation"
                loop_log["stagnation_detected"] = True
                self._emit_generated_tools_event(
                    {
                        "event": "sage_repair_loop_stalled",
                        "mode": "sage",
                        "tool_name": generated_tool_name,
                        "attempt": attempt,
                        "verdict": verdict.verdict,
            ...
            if attempt < max_total:
                ...
                if repair_action == "stay_in_family":
                    ...
                    self._emit_generated_tools_event(
                        {
                            "event": "sage_repair_loop_retry_same_plan_after_alias_repair",
                            "mode": "sage",
                            "tool_name": generated_tool_name,
                            "attempt": attempt,
                    ...
                    self._emit_generated_tools_event(
                        {
                            "event": "sage_repair_loop_repair_scheduled",
                            "mode": "sage",
                            "tool_name": generated_tool_name,
                            "attempt": attempt,
                            "next_attempt": attempt + 1,
                ...
                if rewritten_query_plan is not None:
                    ...
                    self._emit_generated_tools_event(
                        {
                            "event": "sage_repair_loop_plan_rewritten",
                            "mode": "sage",
                            "tool_name": generated_tool_name,
                            "attempt": attempt,
                            "next_attempt": attempt + 1,
                    ...
                    previous_query_plan = copy.deepcopy(working_query_plan)
                    previous_alias_assignments = (
                        self._extract_anchor_alias_assignments(working_query_plan)
                    )
                    try:
                        refreshed_query_plan = self._generate_sage_query_plan(
                            task_question=task_question,
                            grounding_card=working_grounding_card,
                            relation_grounding=working_relation_grounding,
                            generated_tool_name=generated_tool_name,
                        ...
                        self._emit_generated_tools_event(
                            {
                                "event": "sage_repair_loop_plan_refresh_failed",
                                "mode": "sage",
                                "tool_name": generated_tool_name,
                                "attempt": attempt,
                                "next_attempt": attempt + 1,
                        ...
                        if rewritten_after_alias_refresh is not None:
                            ...
                            _sync_return_query_plan(working_query_plan)
                        working_query_plan = self._preserve_chain_query_shape_on_refresh(
                            previous_query_plan=previous_query_plan,
                            refreshed_query_plan=working_query_plan,
                        )
                        _sync_return_query_plan(working_query_plan)
                        ...
                        self._emit_generated_tools_event(
                            {
                                "event": "sage_repair_loop_plan_refreshed",
                                "mode": "sage",
                                "tool_name": generated_tool_name,
                                "attempt": attempt,
                                "next_attempt": attempt + 1,
        ...
        if best_executing is not None:
            ...
            if not loop_log.get("stop_reason") and self._should_accept_best_executing_candidate(
                candidate_metadata=best_executing
            ):
                loop_log["accepted_attempt"] = "best_executing"
                loop_log["final_verdict"] = "accepted_best_effort"
                loop_log["stop_reason"] = "accepted_best_effort"
                self._emit_generated_tools_event(
                    {
                        "event": "sage_repair_loop_salvaged",
                        "mode": "sage",
                        "tool_name": generated_tool_name,
            ...
            self._emit_generated_tools_event(
                {
                    "event": "sage_repair_loop_fallback",
                    "mode": "sage",
                    "tool_name": generated_tool_name,
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_repair_loop_all_failed",
                "mode": "sage",
                "tool_name": generated_tool_name,
                "total_attempts": loop_log["total_attempts"],
                "repair_used": loop_log["repair_used"],
                "stop_reason": loop_log.get("stop_reason") or "repair_budget_exhausted",
            }
        )
        if not loop_log.get("stop_reason"):
            loop_log["stop_reason"] = "repair_budget_exhausted"
        _sync_return_query_plan(working_query_plan)
        ...
        *,
        ...
        if not selected_repairs:
            if repair_events:
                self._emit_generated_tools_event(
                    {
                        "event": "sage_alias_repair_probe_attempt",
                        "mode": "sage",
                        "applied": False,
        ...
        repaired_grounding_card = self._build_sage_grounding_card(
            task_question,
            relation_grounding=relation_grounding,
            alias_overrides=alias_overrides,
        )
    ...
    def _token_search_alias_preserves_surface_semantics(
        ...
        *,
        ...
        if not surface_tokens or not candidate_tokens:
            return False
        overlap_count = self._count_semantic_alias_token_overlap(
            surface_tokens=surface_tokens,
            candidate_tokens=candidate_tokens,
        )
        if len(surface_tokens) <= 1:
            return overlap_count >= 1
        return overlap_count >= min(2, len(surface_tokens))
        ...
        *,
        ...
        for candidate_payload in candidate_counts:
            ...
            close_match = int(
                ...
                and bool(normalized_candidate)
                and self._alias_surface_texts_are_close(
                    normalized_surface,
                    normalized_candidate,
                )
            )
            score_tuple = (
                max(prefix_match, close_match),
                overlap_count,
                surface_coverage,
                -entity_count,
            ...
            if prefix_match or close_match:
    ...
    def _tokenize_alias_surface(self, text: str) -> set[str]:
        ...
        for raw_token in re.findall(r"[A-Za-z0-9&'.-]+", str(text or "").lower()):
            if len(raw_token) <= 2 or raw_token in {"the", "and", "of", "for", "inc", "ltd"}:
                continue
            singular = self._singularize_phrase(raw_token).lower().strip()
            canonical_token = singular if singular and len(singular) > 2 else raw_token
            if canonical_token:
                tokens.add(canonical_token)
        return tokens

    def _count_semantic_alias_token_overlap(
        self,
        *,
        ...
        if not surface_tokens or not candidate_tokens:
            return 0
        remaining_candidates = set(candidate_tokens)
        overlap_count = 0
        for surface_token in surface_tokens:
            match = next(
                (
                    candidate_token
                    for candidate_token in sorted(remaining_candidates)
                    if self._alias_tokens_are_close(
                        surface_token,
                        candidate_token,
                    )
                ),
    ...
    def _alias_tokens_are_close(self, left: str, right: str) -> bool:
        ...
        if normalized_left == normalized_right:
            return True
        if self._singularize_phrase(normalized_left) == self._singularize_phrase(
            normalized_right
        ):
            return True
        if abs(len(normalized_left) - len(normalized_right)) > 2:
            return False
        return SequenceMatcher(None, normalized_left, normalized_right).ratio() >= 0.82

    def _alias_surface_texts_are_close(self, left: str, right: str) -> bool:
        normalized_left = self._normalize_alias_surface_text(left)
        normalized_right = self._normalize_alias_surface_text(right)
        if not normalized_left or not normalized_right:
            return False
        if normalized_left == normalized_right:
            return True
        if abs(len(normalized_left) - len(normalized_right)) > 3:
            return False
        return SequenceMatcher(None, normalized_left, normalized_right).ratio() >= 0.84
    ...
    def _augment_grounding_with_dynamic_probe_on_path_failure(
        ...
        *,
        ...
        if not anchor_probe_results:
            return (
                self._build_sage_grounding_card(
                    task_question,
                    relation_grounding=relation_grounding,
                    alias_overrides=self._extract_anchor_alias_overrides(query_plan),
                ),
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_dynamic_grounding_repair_augmented",
                "mode": "sage",
                "added_relations": [
    ...
    def _refresh_dynamic_grounding_after_anchor_alias_change(
        ...
        *,
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_dynamic_grounding_alias_refresh",
                "mode": "sage",
        ...
        *,
        ...
        if (
            ...
            if anchor_dynamic_candidates:
                ...
                self._emit_generated_tools_event(
                    {
                        "event": "sage_structural_repair_anchor_dynamic_candidates_added",
                        "mode": "sage",
                        "query_shape": query_shape,
            ...
            if pivot_dynamic_candidates:
                ...
                self._emit_generated_tools_event(
                    {
                        "event": "sage_structural_repair_pivot_dynamic_candidates_added",
                        "mode": "sage",
                        "query_shape": query_shape,
        ...
        if (
            self._runtime_single_anchor_pivot_dynamic_enabled()
            and query_shape == "single_anchor_lookup"
            and verdict.verdict == "repairable_anchor_path_empty"
            and "anchor_paths_live_but_projection_empty" in (verdict.reasons or [])
        ):
            pivot_dynamic_candidates = self._probe_dynamic_relation_candidates_for_live_pivots(
                query_plan=query_plan,
                anchor_probe_results=anchor_probe_results,
            ...
            if pivot_dynamic_candidates:
                ...
                self._emit_generated_tools_event(
                    {
                        "event": "sage_single_anchor_pivot_dynamic_candidates_added",
                        "mode": "sage",
                        "query_shape": query_shape,
                        "added_relation_candidates": pivot_dynamic_candidates,
                    }
                ...
                feedback.extend(
                    [
                        "plausibility_feedback:single_anchor_pivot_dynamic_grounding_augmented — the anchor-side path is live but the projection hop is empty, so second-hop pivot relations were added for single-anchor repair",
                        "repair_hint:prefer_live_pivot_projection_relations — when a live anchor hop reaches concrete pivot entities, prefer grounded second-hop relations from those pivots before reusing a semantically weak projection family",
                    ]
                )
        ...
        if verdict.verdict == "repairable_bad_count_set":
            ...
            if pivot_candidates:
                ...
                self._emit_generated_tools_event(
                    {
                        "event": "sage_structural_repair_candidates_added",
                        "mode": "sage",
                        "query_shape": query_shape,
    ...
    def _determine_pivot_probe_target_role(
        ...
        *,
        ...
        query_shape = str(query_plan.get("query_shape") or "").strip().lower()
        answer_mode = str(query_plan.get("answer_mode") or "entity").strip().lower()
        if query_shape == "single_anchor_lookup" and answer_mode == "entity":
            return "answer"
    ...
    def _suppress_dead_grounded_relations(
        ...
        *,
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_dead_grounding_relation_suppressed",
                "mode": "sage",
    ...
    def _validate_sage_query_candidate(
        self,
        *,
        raw_output: str,
        generated_code: str,
    ...
    def _validate_query_constraint_bindings_against_plan(
        ...
        *,
        ...
        for variable_name in self._extract_single_use_helper_variables(
            query_text=query_text,
            query_plan=query_plan,
        ):
    ...
    def _extract_single_use_helper_variables(
        self,
        *,
        query_text: str,
        query_plan: Mapping[str, Any],
        ...
        candidates: list[str] = []
        primary_tokens = self._primary_structural_variable_tokens(
            query_text=query_text,
            query_plan=query_plan,
        )
        projected_variables = {
        ...
        for line in self._extract_structural_triple_lines(query_text):
            for match in re.finditer(
                ...
                variable_token = self._normalize_variable_token(variable_name)
                if (
                    not variable_name
                ...
                if structural_count == 1 and total_count <= 1:
                    if self._query_variable_is_allowed_terminal_leaf(
                        query_text=query_text,
                        variable_name=variable_name,
    ...
    def _primary_structural_variable_tokens(
        self,
        *,
        query_text: str,
        query_plan: Mapping[str, Any],
        ...
        tokens = {
            self._normalize_variable_token(query_plan.get("shared_answer_variable")),
            self._normalize_variable_token(query_plan.get("candidate_set_variable")),
            self._normalize_variable_token(query_plan.get("count_set_variable")),
        }
        projected_variables = self._extract_projected_variables(query_text)
        tokens.update(
            self._normalize_variable_token(variable_name)
            for variable_name in projected_variables
        )
        return {token for token in tokens if token}

    def _query_variable_is_allowed_terminal_leaf(
        self,
        *,
        query_text: str,
        variable_name: str,
        primary_tokens: Collection[str],
    ) -> bool:
        if not self._runtime_terminal_leaf_relaxation_enabled():
            return False
        variable_token = self._normalize_variable_token(variable_name)
        if not variable_token or not primary_tokens:
            return False
        triple_pattern = re.compile(
            r"^(?P<subject>\?[A-Za-z_][A-Za-z0-9_]*)\s+fb:[^\s]+\s+(?P<object>\?[A-Za-z_][A-Za-z0-9_]*)\s*\.$",
            flags=re.IGNORECASE,
        )
        for line in self._extract_structural_triple_lines(query_text):
            ...
            if match is None:
                continue
            object_token = self._normalize_variable_token(match.group("object"))
            if object_token != variable_token:
                continue
            subject_token = self._normalize_variable_token(match.group("subject"))
            return subject_token in primary_tokens
        return False
    ...
    def _build_sage_grounding_card(
        self,
        task_question: str,
        *,
        relation_grounding: Optional[Sequence[Mapping[str, str]]] = None,
        ...
        if derived_entities:
            ...
            for raw_entity in derived_entities:
                normalized_entity = self._normalize_grounding_entity(raw_entity)
    ...
    def _infer_query_shape(
        ...
        *,
        ...
        if (
            anchor_like_count == 1
            and self._runtime_broader_single_anchor_chain_inference_enabled()
            and self._looks_like_single_anchor_chain_question(
                question_text=lower_text,
                answer_target_phrase=answer_target_phrase,
                question_inputs=normalized_question_inputs,
            )
        ):
    ...
    def _runtime_broader_single_anchor_chain_inference_enabled(self) -> bool:
        return os.getenv("SAGE_RUNTIME_BROADER_SINGLE_ANCHOR_CHAIN_INFERENCE", "0").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def _looks_like_single_anchor_chain_question(
        self,
        *,
        question_text: str,
        answer_target_phrase: str,
        ...
        lower_text = str(question_text or "").strip().lower()
        answer_target_lower = str(answer_target_phrase or "").strip().lower()
        if not lower_text:
            return False
        if any(
            ...
            return True
        if re.search(r"\bdoes\b.+\bhave\b", lower_text):
            return True
        if re.search(
            r"\b(?:that|who|which)\b.+\b("
            r"performed|composed|released|produced|developed|designed|"
            r"written|wrote|played|sang|starred"
            r")\b",
            lower_text,
        ):
            return True
        has_class_phrase = any(
            str(item.get("kind") or "").strip() == "class_phrase"
            for item in question_inputs
            if isinstance(item, Mapping)
        )
        if has_class_phrase and " of " in lower_text and len(answer_target_lower.split()) >= 2:
            return True
        return False
    ...
    def _build_grounded_relation_candidates_with_dynamic_fallback(
        ...
        *,
        ...
        if curated:
            self._emit_generated_tools_event(
                {
                    "event": "sage_grounding_curated_hit",
                    "mode": "sage",
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_grounding_curated_miss",
                "mode": "sage",
        ...
        if dynamic:
            self._emit_generated_tools_event(
                {
                    "event": "sage_grounding_dynamic_hit",
                    "mode": "sage",
            ...
            self._emit_generated_tools_event(
                {
                    "event": "sage_grounding_dynamic_miss",
                    "mode": "sage",
    ...
    def _probe_answer_class_dynamic_candidates(
        ...
        *,
        ...
        if type_dynamic:
            self._emit_generated_tools_event(
                {
                    "event": "sage_grounding_answer_class_dynamic_hit",
                    "mode": "sage",
    ...
    def _run_anchor_existence_probes(
        ...
        *,
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_anchor_probe_results",
                "mode": "sage",
                "probes": [
                    {
    ...
    def _probe_dynamic_relation_candidates_for_anchor(
        ...
        *,
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_grounding_dynamic_probe_start",
                "mode": "sage",
        ...
        predicate_filter = ""
        if self._runtime_dynamic_probe_fb_filter_enabled():
            predicate_filter = (
                f'  FILTER(STRSTARTS(STR(?p), "{self._FB_NS}"))\n'
            )
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_grounding_dynamic_probe_raw",
                "mode": "sage",
                "anchor": anchor_entity,
                "probe_alias": probe_alias,
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_grounding_dynamic_candidates_selected",
                "mode": "sage",
                "anchor": anchor_entity,
    ...
    def _is_noise_predicate(self, iri: str) -> bool:
        ...
        for prefix in self._NOISE_PREDICATE_PREFIXES:
            if (
                prefix == "http://rdf.freebase.com/ns/base."
                and self._runtime_dynamic_probe_allow_base_enabled()
            ):
                continue
    ...
    def _capture_sage_query_artifacts(
        self,
        *,
        generated_tool_name: str,
        generated_code: str,
        ...
        if query_plan is not None:
            plan_path = self._capture_sage_query_plan_artifact(
                generated_tool_name=generated_tool_name,
                query_plan=query_plan,
            )
        projected_variables = self._extract_projected_variables(query_text)
        self._emit_generated_tools_event(
            {
                "event": "sage_query_artifact_saved",
                "mode": "sage",
                "tool_name": generated_tool_name,
    ...
    def _refresh_query_artifacts_from_invocation(
        ...
        *,
        ...
        self._emit_generated_tools_event(
            {
                "event": "sage_query_runtime_captured",
                "mode": "sage",
                "tool_name": generated_tool_name,
    ...
    def _get_sage_query_artifact_dir(self) -> Path:
        output_dir = Path(
            os.environ.get("LIFELONG_OUTPUT_DIR", "outputs/sage_runtime")
        )
        artifact_dir = output_dir / "sage_query_artifacts"
    ...
    def _extract_sparql_query_texts(self, generated_code: str) -> list[str]:
        ...
        class _SetQueryCollector(ast.NodeVisitor):
            def __init__(self, outer: "SAGEAgentController") -> None:
                self.outer = outer
    ...
    def _build_sage_query_execution_event(
        self,
        *,
        generated_tool_name: str,
        query_artifacts: Mapping[str, str],
        ...
        return {
            "event": "sage_query_execution_result",
            "mode": "sage",
            "tool_name": generated_tool_name,
    ...
    def _log_macro_result(self, content: str) -> None:
        ...
        self._emit_generated_tools_event(
            {
                "event": "invoke",
                "tool_name": tool_name or SAGE_BENCHMARK_BRIDGE_TOOL_NAME,
    ...
    def _build_tool_semantic_description(
        ...
        *,
        ...
        if not answer_target:
            return base_description
        if artifact_type == "count_scalar":
            return f"count of {answer_target} returned by the executed SAGE query"
        if artifact_type == "entity_id":
            return f"single {answer_target} returned by the executed SAGE query"
        if artifact_type == "entity_set":
            return f"set of {answer_target} returned by the executed SAGE query"
        if artifact_type == "boolean":
            return f"boolean result for {answer_target} returned by the executed SAGE query"
        if artifact_type in {"scalar_literal", "text_literal"}:
            return f"value for {answer_target} returned by the executed SAGE query"
        return base_description
    ...
    def _build_tool_proof_hint(
        ...
        *,
        ...
        if query_shape == "superlative_chain":
            ...
            if relation:
                return (
                    f"SAGE query already selected the single {target_text} with the "
                    f"{direction_text} value of {relation} from the grounded candidate set."
                )
            return (
                f"SAGE query already applied the requested superlative and selected the "
        ...
        if answer_mode == "count" or query_shape.startswith("count_"):
            if relation_summary:
                return (
                    "SAGE query already computed the final count from the exact relation constraints listed above."
                )
            return "SAGE query already computed the final count over the grounded answer set."
        ...
        if (
            ...
            target_text = answer_target or "answer entity"
            return (
                f"SAGE query already narrowed the grounded {selected_query_variable} "
    ...
    def _build_tool_advisory_text(
        ...
        *,
        ...
        lines = [
            f"SAGE tool status: {semantics.get('tool_result_status')}.",
        ]
        ...
        if semantics.get("tool_result_useful_intermediate"):
            ...
            lines.append(
                "Ignore the SAGE tool output if it does not help, and continue solving manually."
            )
    ...
    def _build_repair_loop_failure_advisory(
        ...
        *,
        ...
        lines = [
            "SAGE tool generation did not produce a trustworthy tool result.",
            f"Failure reason: {repair_failure_reason}.",
        ]
        if last_reasons:
            lines.append("Recent SAGE repair hints: " + "; ".join(last_reasons) + ".")
        lines.append(
            "Continue manually from the original task. Use the SAGE hints only if they help."
        )
        return " ".join(line.strip() for line in lines if line.strip())

    def _build_sage_plan_failure_advisory(
        self,
        *,
        failure_reason: str,
    ) -> str:
        cleaned_reason = str(failure_reason or "sage_query_plan_invalid").strip()
        lines = [
            "SAGE tool planning produced an invalid family-constrained query plan.",
    ...
    def _build_adapter_context(
        ...
        *,
        ...
        return BenchmarkAdapterContext(
            ...
            bridge_tool_name=SAGE_BENCHMARK_BRIDGE_TOOL_NAME,
            family_name=str((query_plan or {}).get("query_shape") or "").strip(),
            query_shape=str((query_plan or {}).get("query_shape") or "").strip(),
        )
    ...
    def _ensure_bridge_tool(self, tool_name: str) -> str:
        ...
        if not tool_path.exists():
            tool_code = build_sage_benchmark_bridge_tool_code()
            ...
            self._emit_generated_tools_event(
                {
                    ...
                    "description": "SAGE benchmark adapter bridge macro.",
                    "docstring": "Bridge typed SAGE artifacts into deterministic benchmark variables.",
                    "tool_type": "utility",
                    ...
                    "input_schema": {
                        ...
                        "properties": {
                            "sage_artifact_type": {"type": "string"},
                            "sage_artifact_value": {},
                            "sage_artifact_source": {"type": "string"},
                            "sage_artifact_diagnostics": {"type": "object"},
                            "run_id": {"type": "string"},
                    ...
                    "optional_keys": [
                        "sage_artifact_source",
                        "sage_artifact_diagnostics",
                            "sage_semantic_description",
                            "sage_solves_task",
                            "sage_trusted_for_materialization",
                            "sage_tool_status",
