...
def extract_semantic_failure_labels(
    *,
    ...
    for payload in load_json_lines(generated_tools_path):
        ...
        elif payload.get("event") == "pal_repair_loop_rejected":
...
def family_lock_violations(
    *,
    ...
    for payload in load_json_lines(run_dir / "generated_tools.log"):
        ...
        if event == "pal_query_plan_generated":
        ...
        elif event in {"pal_reusable_tool_selected", "pal_attempt_decision", "pal_attempt_decision_finalized"}:
            selected_family = str(
    ...
    *,
    ...
    if not typed_outcome:
        summary_primary_failure_kind = str(
            summary.get("pal_primary_failure_kind") or ""
        ).strip()
        if summary_primary_failure_kind:
            typed_outcome = {
                "primary_failure_kind": summary_primary_failure_kind,
                "stop_reason": str(summary.get("pal_stop_reason") or "").strip(),
                "completion_state": str(
                    summary.get("pal_completion_state") or ""
                ).strip(),
                "repair_attempt_count": int(
                    summary.get("pal_repair_attempt_count") or 0
                ),
            }
...
def baseline_satisfies_locked_family(
    ...
    return not any(
        label.startswith("pal_query_plan_invalid:family_compare_locked_query_shape:")
        for label in labels
    )
