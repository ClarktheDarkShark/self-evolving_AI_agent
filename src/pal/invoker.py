...
class PALInvokerError(RuntimeError):
    pass


@dataclass(frozen=True)
class PALInvocationResult:
    success: bool
...
def _build_safe_import(sparqlwrapper_module: Any):
    ...
    def _safe_import(
        ...
        if level != 0:
            raise PALInvokerError("relative_import_not_allowed")
        if name not in allowed_modules:
            raise PALInvokerError(f"import_not_allowed:{name}")
...
def _build_redirected_sparqlwrapper_module(
    ...
    *,
    ...
    if wrapper_cls is None:
        raise PALInvokerError("SPARQLWrapper_class_missing")
...
def _build_invocation(
    ...
    if len(parameters) > 1:
        raise PALInvokerError("entrypoint_accepts_too_many_parameters")
    if not parameters:
        return [], {}
    ...
    raise PALInvokerError("unsupported_entrypoint_signature")
...
def _coerce_json_payload(raw_payload: Any) -> dict[str, Any]:
    try:
        ...
        raise PALInvokerError("payload_not_json_serializable") from exc
    if not isinstance(json_payload, dict):
        raise PALInvokerError("payload_must_be_json_object")
    return json_payload


def execute_pal_code(
    code: str,
    *,
    endpoint_url: str = DEFAULT_SPARQL_ENDPOINT_URL,
    timeout_s: float = 10.0,
) -> dict[str, Any]:
    result = execute_pal_code_with_result(
        code,
        endpoint_url=endpoint_url,
        timeout_s=timeout_s,
    )
    if not result.success or result.payload is None:
        raise PALInvokerError(result.error or "pal_execution_failed")
    return result.payload


def execute_pal_code_with_result(
    code: str,
    *,
    endpoint_url: str = DEFAULT_SPARQL_ENDPOINT_URL,
    timeout_s: float = 10.0,
) -> PALInvocationResult:
    parsed_program = parse_pal_code(code)
    return invoke_pal_program(
        parsed_program,
        endpoint_url=endpoint_url,
        timeout_s=timeout_s,
    )
...
def build_pal_execution_failure_payload(
    ...
    *,
    ...
    return {
        ...
        "pal_failure_kind": invocation_result.failure_kind or "execution_error",
        "pal_error": invocation_result.error or "pal_execution_failed",
        "endpoint_url": endpoint_url,
        "pal_diagnostics": dict(invocation_result.diagnostics),
    }
