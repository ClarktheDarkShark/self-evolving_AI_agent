# SAGE Retained Baseline

This document defines the current retained SAGE runtime surface, the active run modes that should be treated as canonical, and the code/config paths that are experimental or archived.

## Retained Runtime

These files define the live retained SAGE path and should remain the reference surface:

- `scripts/run_all_with_servers.py`
- `src/agents/instance/sage_agent_controller.py`
- `src/sage/plausibility_validator.py`
- `src/sage/reusable_tool_families.py`
- `src/sage/family_policy_evolution.py`
- `src/tasks/instance/knowledge_graph/api.py`

Retained behavior in this surface includes:

- SAGE-first runtime with manual fallback in standard mode
...
- `src/sage/candidate_compare.py`
- `tests/test_candidate_compare.py`

These support:

- inline family evolution gating
- stage-A SAGE-only candidate screening
...
- tracked historical `sage_batch_*.yaml` configs in the KG config directory
...
Do not treat historical `outputs/overnight_*` or `sage_batch_*` configs as the current baseline.

## Experimental Flags

Retained baseline should keep these experimental toggles off unless a controlled experiment explicitly needs them:

- `SAGE_RUNTIME_SINGLE_ANCHOR_PIVOT_DYNAMIC`
- `SAGE_RUNTIME_SINGLE_ANCHOR_ANSWER_BINDING`
- `SAGE_RUNTIME_SINGLE_ANCHOR_TARGET_SEMANTICS`
- `SAGE_RUNTIME_SINGLE_ANCHOR_LOW_TRUST_DYNAMIC`
- `SAGE_RUNTIME_SINGLE_ANCHOR_CHAIN_LOW_TRUST_DYNAMIC`
- `SAGE_RUNTIME_PRESERVE_CHAIN_QUERY_SHAPE_ON_REFRESH`
- `SAGE_RUNTIME_JOINED_COUNT_TARGET_BOUNDARY`
- `SAGE_RUNTIME_DYNAMIC_PROBE_FB_FILTER`
- `SAGE_RUNTIME_DYNAMIC_PROBE_ALLOW_BASE`
- `SAGE_RUNTIME_REUSABLE_SWAP_RENDER_CANONICALIZATION`
- `SAGE_INLINE_FAMILY_STAGE_A_SCREEN`
- `SAGE_FAMILY_POLICY_COMPARE_LOCK_FAMILY`
- `SAGE_FAMILY_POLICY_STRICT_UPDATE_MAPPING`
- `SAGE_FAMILY_POLICY_DEDUP_SIGNATURE`
- `SAGE_FAMILY_POLICY_STRUCTURED_SUCCESS_FEATURES`
