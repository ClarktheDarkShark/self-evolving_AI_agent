...
from src.sage.invoker import execute_sage_code_with_result
from src.sage.invoker import _classify_invocation_exception
...
def test_unresolved_artifact_is_not_bridge_materialized() -> None:
    artifact = PalExecutionArtifact(
        raw_result={"sage_execution_status": "error"},
...
def test_bridge_tool_refuses_unresolved_artifact() -> None:
    ...
    bridge_code = build_sage_benchmark_bridge_tool_code()
    ...
    result = namespace["run"](
        {
            "variable_list": [],
            "sage_artifact_type": "unresolved",
            "sage_artifact_value": "endpoint_timeout",
        }
    )
...
def test_bridge_tool_returns_semantic_contract_fields() -> None:
    ...
    result = namespace["run"](
        {
            "variable_list": [],
            "sage_artifact_type": "count_scalar",
            "sage_artifact_value": "4",
            "sage_selected_query_variable": "count",
            "sage_binding_count": 1,
            "sage_unique_value_count": 1,
            "sage_value_preview": ["4"],
            "sage_row_preview": [{"count": "4"}],
            "sage_semantic_description": "count result returned by the SAGE query",
            "sage_solves_task": True,
            "sage_trusted_for_materialization": True,
            "sage_proof_hint": "SAGE query already computed the final count.",
            "sage_answer_cardinality_hint": "single",
            ...
            "sage_repair_caveat": "This answer depends on a repaired alternate relation; verify semantic equivalence.",
            "sage_confidence": 1.0,
        }
    )

    assert result["status"] == "SUCCESS"
    assert result["semantic_description"] == "count result returned by the SAGE query"
    ...
    assert result["proof_hint"] == "SAGE query already computed the final count."
...
def test_bridge_tool_returns_partial_advisory_contract_fields() -> None:
    ...
    result = namespace["run"](
        {
            "variable_list": [],
            "sage_artifact_type": "entity_set",
            "sage_artifact_value": ["m.1", "m.2"],
            "sage_selected_query_variable": "candidate_set",
            "sage_binding_count": 2,
            "sage_unique_value_count": 2,
            "sage_value_preview": ["m.1", "m.2"],
            "sage_row_preview": [{"candidate_set": "m.1", "name": "First"}, {"candidate_set": "m.2", "name": "Second"}],
            "sage_semantic_description": "bounded candidate set returned by the SAGE query",
            "sage_solves_task": False,
            "sage_trusted_for_materialization": False,
            "sage_tool_status": "partial",
            "sage_failure_reason": "needs_manual_disambiguation",
            "sage_confidence": 0.4,
        }
    )

    assert result["status"] == "PARTIAL"
    assert result["semantic_description"] == "bounded candidate set returned by the SAGE query"
...
def test_macro_result_summary_surfaces_only_trusted_final_pointer() -> None:
    trusted_summary = KnowledgeGraph._build_macro_result_summary(
        tool_name="sage_benchmark_bridge_macro",
        result={
            "status": "SUCCESS",
            "final_variable": "#3",
            "observation": "ok",
            ...
            "proof_hint": "SAGE query already computed the final count.",
    ...
    assert "Proof hint: SAGE query already computed the final count." in trusted_summary
    assert (
...
def test_sage_invoker_timeout_terminates_stuck_program() -> None:
    code = """###QUERY_START
...
###QUERY_END"""
    ...
    result = execute_sage_code_with_result(code, timeout_s=0.2)
