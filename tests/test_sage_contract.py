...
from src.sage.invoker import SAGEInvocationResult
...
from src.sage.policy_contracts import FamilyPolicyBundle, build_trust_contract_evaluation
...
def test_family_policy_candidate_uses_current_session_status_when_available(
        ...
        def create_candidate_update(self, **kwargs):
            ...
            return type(
                ...
                (),
                {
                    ...
                    "reason_for_change": "family_failure:sage_query_not_accepted:repairable_bad_count_set",
    ...
    controller._maybe_record_family_policy_candidate(
        generated_tool_name="sage_sparql_query_tool_demo",
        query_plan={"query_shape": "count_over_joined_set"},
        failure_reason="sage_query_not_accepted:repairable_bad_count_set",
...
def test_normalize_sage_query_plan_reclassifies_direct_count_with_downstream_filter() -> None:
    controller = _make_controller()

    normalized = controller._normalize_sage_query_plan(
        {
            "answer_mode": "count",
            "answer_type": "count",
            "query_shape": "count_over_direct_relation",
...
def test_normalize_sage_query_plan_reclassifies_two_hop_direct_count() -> None:
    controller = _make_controller()
...
def test_normalize_sage_query_plan_repairs_nonstructural_joined_count_variables() -> None:
    controller = _make_controller()
...
def test_normalize_joined_count_plan_keeps_bridge_candidate_and_count_target_distinct() -> None:
    ...
    plan = controller._normalize_sage_query_plan(
        {
            "answer_mode": "count",
            "answer_type": "count",
...
def test_validate_sage_execution_accepts_multi_anchor_mid_pins_without_probe_results() -> None:
...
def test_validate_sage_execution_treats_constraint_value_probe_miss_as_optional() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "query_shape": "count_over_joined_set",
...
def test_validate_sage_execution_requires_explicit_constraint_value_anchor() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "film characters",
...
def test_validate_sage_execution_respects_relation_selection_hint_for_direct_count() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "songwriters",
...
SELECT (COUNT(DISTINCT ?candidate_set) AS ?count) WHERE {
  ...
  ?candidate_set fb:language.language_dialect.language ?anchor .
    ...
    generated_code = f'query = """{query_text}"""\nwrapper.setQuery(query)\n'
    original_result = SAGEInvocationResult(
        success=True,
        payload={
            "results": {
                "bindings": [
    ...
    retried_result = SAGEInvocationResult(
        success=True,
        payload={
            "results": {
                "bindings": [
...
def test_validate_sage_execution_treats_anchor_probe_as_optional_for_type_set_only_count_plan() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "query_shape": "count_over_direct_relation",
...
def test_infer_query_shape_broader_chain_inference_for_does_have_question(monkeypatch) -> None:
    monkeypatch.setenv("SAGE_RUNTIME_BROADER_SINGLE_ANCHOR_CHAIN_INFERENCE", "1")
    controller = _make_controller()
...
def test_repair_loop_rejection_is_fail_closed() -> None:
    ...
    assert (
        controller._ensure_repair_loop_accepted(
            generated_tool_name="sage_sparql_query_tool_test",
            repair_loop_log={
                "total_attempts": 3,
                "repair_used": True,
...
def test_repair_loop_rejection_emits_typed_finalized_failure() -> None:
    ...
    controller._emit_repair_loop_rejection_finalization(
        ...
        repair_failure_reason="sage_query_not_accepted:repairable_anchor_path_empty",
    )

    assert emitted_events[-1]["event"] == "sage_attempt_decision_finalized"
...
def test_repair_loop_stagnation_cuts_off_repeated_state() -> None:
    controller = _make_controller()
    controller._generate_validated_sage_candidate = lambda **kwargs: ("def solve(endpoint_url):\n    return {}\n", {})
    ...
    controller._augment_grounding_with_dynamic_probe_on_path_failure = (
        lambda **kwargs: ("SAGE grounding hints:", kwargs["relation_grounding"], [])
    )
    ...
    controller._generate_sage_query_plan = lambda **kwargs: kwargs.get("relation_grounding") and {
        "answer_mode": "count",
        "query_shape": "count_over_direct_relation",
    ...
    with patch.object(
        ...
        _code, _result, loop_log = controller._run_sage_repair_loop(
            ...
            grounding_card="SAGE grounding hints:",
            query_plan=query_plan,
            generated_tool_name="sage_sparql_query_tool_test",
...
def test_repair_loop_probes_nonempty_dynamic_single_anchor_lookup_for_disambiguation() -> None:
    ...
    controller._generate_validated_sage_candidate = (
...
def test_repair_loop_plan_refresh_reapplies_resolved_anchor_ids() -> None:
    ...
    def _generate_validated_sage_candidate(**kwargs):
        call_state["count"] += 1
    ...
    controller._generate_validated_sage_candidate = _generate_validated_sage_candidate
    controller._extract_sparql_query_texts = (
    ...
    controller._generate_sage_query_plan = lambda **kwargs: {
        "answer_mode": "count",
        "query_shape": "count_over_direct_relation",
...
def test_repair_loop_salvage_restores_best_executing_query_plan() -> None:
    ...
    controller._generate_validated_sage_candidate = _generate_candidate
    controller._extract_sparql_query_texts = (
        lambda code: [
            "SELECT (COUNT(DISTINCT ?candidate_set) AS ?count) WHERE { "
    ...
    controller._validate_sage_query_candidate = lambda **kwargs: []
...
def test_family_policy_candidate_uses_final_rejected_plan_instead_of_best_executing_salvage(
    ...
    controller._maybe_record_family_policy_candidate(
        ...
        failure_reason="sage_query_not_accepted:rejected_dangerous_overreach",
        repair_loop_log={
...
def test_materialize_adapter_response_fails_closed_for_unresolved_artifact() -> None:
    ...
    with pytest.raises(AgentUnknownException, match="sage_adapter_unresolved_artifact"):
...
def test_materialize_adapter_response_records_query_tool_and_bridge_macro() -> None:
    ...
    response = controller._materialize_adapter_response(
        task_question="Question: ...",
        generated_tool_name="sage_sparql_query_tool_5_deadbeef",
        materialization=BenchmarkMaterialization(
            materialization_type="bridge_action",
            needs_bridge=True,
            bridge_action='Action: execute_macro("sage_benchmark_bridge_macro", {"run_id": "knowledge_graph_5", "state_dir": "outputs/test_state"})',
            bridge_tool_name="sage_benchmark_bridge_macro",
    ...
    assert response.startswith('Action: execute_macro("sage_benchmark_bridge_macro"')
    assert controller._tool_invoked_in_last_inference == (
        "sage_sparql_query_tool_5_deadbeef -> sage_benchmark_bridge_macro"
    )
...
def test_materialize_adapter_response_carries_semantic_bridge_payload() -> None:
    ...
    response = controller._materialize_adapter_response(
        task_question="Question: ...",
        generated_tool_name="sage_sparql_query_tool_6_deadbeef",
        query_plan={
            "query_shape": "count_over_direct_relation",
            "answer_mode": "count",
            "answer_target_phrase": "infectious diseases",
        ...
        materialization=BenchmarkMaterialization(
            materialization_type="bridge_action",
            needs_bridge=True,
            bridge_action='Action: execute_macro("sage_benchmark_bridge_macro", {"run_id": "knowledge_graph_6", "state_dir": "outputs/test_state"})',
            bridge_tool_name="sage_benchmark_bridge_macro",
            ...
            confidence=1.0,
            determinism_level="high",
            semantic_description="count result returned by the SAGE query",
            solves_task=True,
            trusted_for_materialization=True,
        ),
        tool_result_semantics={
            "tool_result_semantic_description": "count of infectious diseases returned by the executed SAGE query",
    ...
    assert response.startswith('Action: execute_macro("sage_benchmark_bridge_macro"')
    assert (
        '"sage_semantic_description": "count of infectious diseases returned by the executed SAGE query"'
        in response
    )
    assert '"sage_solves_task": true' in response
    assert '"sage_trusted_for_materialization": true' in response
    assert '"sage_tool_status": "success"' in response
    assert '"sage_selected_query_variable": "count"' in response
    assert '"sage_binding_count": 1' in response
    assert '"sage_unique_value_count": 1' in response
    assert '"sage_value_preview": ["4"]' in response
    assert '"sage_row_preview": [{"count": "4"}]' in response
    assert (
        '"sage_relation_summary": ["anchor -> count set via biology.organism.diseases_transmitted"]'
        in response
    )
    assert (
        '"sage_selection_basis": "Counts distinct infectious diseases that satisfy the executed relation constraints."'
        in response
    )
    assert (
        '"sage_completeness_hint": "Exhaustive over the exact counted bindings matched by the executed relation constraints."'
        in response
    )
    assert (
        '"sage_proof_hint": "SAGE query already computed the final count from the exact relation constraints listed above."'
        in response
    )
    assert '"sage_answer_cardinality_hint": "single"' in response
    pending = controller._pending_macro_runs["knowledge_graph_6"]
    assert pending["tool_name"] == "sage_benchmark_bridge_macro"
...
def test_extract_macro_pointer_requires_trusted_final_contract() -> None:
    ...
    assert controller._extract_macro_pointer(
        ...
        "Final variable: #3\n"
        "Semantic: count result returned by the SAGE query\n"
        "Solves task: yes\n"
        "Trusted final: yes"
    ) == "#3"
    assert (
        controller._extract_macro_pointer(
            "Macro result: sage_benchmark_bridge_macro -> SUCCESS.\n"
            "Final variable: #3\n"
            "Semantic: bounded candidate set returned by the SAGE query\n"
            "Solves task: no\n"
            "Trusted final: no"
        )
        is None
...
def test_trusted_macro_result_bypasses_manual_solver_in_sage_only_mode(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    controller = _make_controller()
    controller._log_macro_result = lambda content: None
    controller._get_run_id = lambda: "knowledge_graph_7"
    monkeypatch.setenv("SAGE_TOOL_EVOLUTION_SKIP_MANUAL_FALLBACK", "1")
    ...
    assert controller._tool_invoked_in_last_inference == "sage_trusted_macro_bypass"
...
def test_tool_evolution_failure_bypass_raises_without_manual_solver(monkeypatch) -> None:
    ...
    with pytest.raises(
        ...
        match="sage_tool_failure_bypassed:repairable_bad_count_set",
    ):
        controller._bypass_manual_solver_after_tool_failure(
            generated_tool_name="sage_sparql_query_tool_demo",
            failure_reason="repairable_bad_count_set",
            advisory_text=(
                "The repaired SAGE count plan is low-trust and should be recorded "
                "for evolution without waiting for the manual solver."
            ),
        )

    assert controller._tool_invoked_in_last_inference == "sage_tool_failure_bypass"
    assert emitted_events[-1]["event"] == "sage_tool_failure_bypassed"
    assert emitted_events[-1]["tool_name"] == "sage_sparql_query_tool_demo"
    ...
    chat_history.inject(
        ChatHistoryItem(
            ...
            content=(
                "Macro result: sage_benchmark_bridge_macro -> SUCCESS.\n"
                "Final variable: #0\n"
                "Semantic: bounded set of entity ids returned by the SAGE query "
                ...
                "Solves task: yes\n"
                "Trusted final: yes\n"
                "Observation: SAGE benchmark bridge materialized entity_set into a benchmark variable.\n"
                "Confidence: 1.0"
            ),
        )
    )
    ...
    chat_history.inject(
        ChatHistoryItem(
            ...
            content=(
                "Macro result: sage_benchmark_bridge_macro -> SUCCESS.\n"
                "Final variable: #0\n"
                "Semantic: single entity id returned by the SAGE query "
                ...
                "Solves task: yes\n"
                "Trusted final: yes\n"
                "Observation: SAGE benchmark bridge materialized entity_id into a benchmark variable.\n"
                "Confidence: 1.0"
            ),
        )
    )
...
def test_validate_sage_execution_rejects_generic_shared_type_dump() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "entity",
...
def test_should_skip_sage_solver_for_structured_raw_artifacts() -> None:
    controller = _make_controller()
    ...
    assert controller._should_skip_sage_solver_for_raw_artifact(entity_artifact) is True
    assert (
        controller._should_skip_sage_solver_for_raw_artifact(unresolved_artifact)
        is False
    )
...
def test_refresh_dynamic_grounding_after_anchor_alias_change_reprobes_new_alias() -> None:
    controller = _make_controller()
    controller._build_sage_grounding_card = (
...
def test_normalize_sage_query_plan_retags_single_anchor_direct_candidate_set_as_answer() -> None:
    controller = _make_controller()
...
def test_normalize_sage_query_plan_projects_structural_candidate_set_when_single_anchor_type_filter_lacks_answer_path() -> None:
    controller = _make_controller()
...
def test_normalize_sage_query_plan_projects_structural_candidate_set_for_multi_anchor_intersection_without_shared_answer_path() -> None:
    controller = _make_controller()
...
def test_generate_plan_rejects_reused_dead_scaffold_signature() -> None:
    ...
    """
    controller._capture_sage_query_plan_artifact = (
    ...
    with pytest.raises(ValueError, match="sage_query_plan_invalid:query_plan_reused_dead_scaffold_signature"):
        controller._generate_sage_query_plan(
            task_question="Question: what dug dosage form exist for drugs formulated from naloxone and has active ingredient enalaprilat?, Entities: ['Naloxone', 'Enalaprilat']",
            grounding_card="SAGE grounding hints:",
            relation_grounding=[],
            generated_tool_name="sage_sparql_query_tool_test",
...
def test_validate_sage_execution_rejects_multi_entity_result_for_singleton_target() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "entity",
            "answer_target_phrase": "last day",
...
def test_validate_sage_execution_allows_multi_entity_result_for_non_singleton_target() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "entity",
...
def test_normalize_sage_query_plan_coerces_ordered_entity_plan_to_superlative_chain() -> None:
    controller = _make_controller()
...
def test_normalize_sage_query_plan_promotes_terminal_ordering_value_in_chain() -> None:
    controller = _make_controller()
...
def test_normalize_sage_query_plan_coerces_shared_answer_literal_intersection_to_entity() -> None:
    controller = _make_controller()
...
def test_normalize_sage_query_plan_keeps_literal_label_projection_literal() -> None:
    controller = _make_controller()
...
def test_normalize_sage_query_plan_coerces_literal_superlative_to_entity_candidate() -> None:
    controller = _make_controller()
...
def test_query_candidate_rejects_terminal_leaf_when_relaxation_disabled(
    ...
    monkeypatch.setenv("SAGE_RUNTIME_TERMINAL_LEAF_RELAXATION", "0")
    controller = _make_controller()
    query_text = (
        "PREFIX fb: <http://rdf.freebase.com/ns/> "
        "SELECT (COUNT(DISTINCT ?recording) AS ?count) WHERE { "
...
def test_dynamic_probe_query_can_filter_to_freebase_namespace(
    ...
    controller = _make_controller()
    monkeypatch.setenv("SAGE_RUNTIME_DYNAMIC_PROBE_FB_FILTER", "1")
...
def test_dynamic_probe_can_keep_base_namespace_relations_when_enabled(
    ...
    monkeypatch.setenv("SAGE_RUNTIME_DYNAMIC_PROBE_ALLOW_BASE", "1")
...
def test_structural_repair_adds_pivot_dynamic_candidates_for_single_anchor_projection_empty(
    ...
    controller = _make_controller()
    monkeypatch.setenv("SAGE_RUNTIME_SINGLE_ANCHOR_PIVOT_DYNAMIC", "1")
    controller._build_sage_grounding_card = lambda *args, **kwargs: "grounding-card"  # type: ignore[method-assign]
...
def test_validate_sage_query_candidate_rejects_unbound_projected_entity_variable() -> None:
    controller = _make_controller()
...
def test_validate_sage_query_candidate_rejects_ungrounded_type_instance_subject() -> None:
    controller = _make_controller()
...
def test_validate_sage_execution_rejects_positive_count_for_ungrounded_generic_type_plan() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "query_shape": "count_over_direct_relation",
...
def test_validate_sage_execution_rejects_count_when_answer_target_semantics_are_unenforced() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "game expansions",
...
def test_validate_sage_execution_accepts_profession_membership_count_with_profession_like_target() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "songwriters",
...
def test_validate_sage_execution_accepts_prepositional_count_target_when_head_is_enforced() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "contents about higher education",
...
def test_validate_sage_execution_accepts_joined_count_when_shared_filter_counts_grounded_candidate_set() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "breeds",
...
def test_validate_sage_execution_accepts_joined_count_without_shared_filter_when_count_alias_is_detached() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
...
def test_validate_sage_execution_accepts_species_count_target() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "different species",
...
def test_validate_sage_execution_rejects_count_target_dropped_to_relation_hint() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "exhibition subjects",
...
def test_validate_sage_execution_rejects_count_when_counted_variable_only_appears_in_aggregate() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
...
def test_validate_sage_execution_rejects_zero_type_projection_count_with_live_anchor_path() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "language types",
...
def test_validate_sage_execution_rejects_count_of_raw_candidates_for_type_like_target() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "language types",
...
def test_validate_sage_execution_accepts_relation_encoded_type_like_count_target() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "answer_target_phrase": "language types",
...
def test_validate_sage_execution_accepts_direct_type_relation_for_type_like_target() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
...
def test_validate_sage_execution_rejects_entity_result_hitting_query_limit() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "entity",
            "query_shape": "single_anchor_lookup",
...
def test_validate_sage_execution_rejects_generic_type_dump_for_specific_entity_target() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "entity",
            "answer_target_phrase": "meter",
...
def test_validate_sage_execution_repairs_grounded_single_anchor_empty_literal_result() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "literal",
            "answer_target_phrase": "meter",
...
def test_validate_sage_execution_does_not_accept_transport_failed_entity_query() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "entity",
            "query_shape": "single_anchor_lookup",
...
def test_validate_sage_execution_does_not_accept_transport_failed_count_query() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "count",
            "query_shape": "count_over_direct_relation",
...
def test_validate_sage_execution_repairs_missing_selected_head_var_in_bindings() -> None:
    verdict = validate_sage_execution(
        query_plan={
            "answer_mode": "literal",
            "query_shape": "single_anchor_lookup",
