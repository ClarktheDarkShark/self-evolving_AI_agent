...
ENV_RUN_SAMPLE_TIMEOUT_SECONDS = "PAL_KG_BATCH_RUN_TIMEOUT_SECONDS"
DEFAULT_RUN_SAMPLE_TIMEOUT_SECONDS = 300
PAL_ONLY_BYPASS_ENV = "PAL_TOOL_EVOLUTION_SKIP_MANUAL_FALLBACK"
EVOLUTION_DISABLED_ENVS = (
    "PAL_ENABLE_FAMILY_POLICY_EVOLUTION",
    "PAL_ENABLE_FAMILY_POLICY_PROMOTION",
    "PAL_ENABLE_STANDARD_FAMILY_EVOLUTION",
)
COMPARE_LOCK_FAMILY_ENV = "PAL_FAMILY_POLICY_COMPARE_LOCK_FAMILY"
...
def _extract_pal_typed_summary(
    *,
    ...
    for payload in _load_json_lines(generated_tools_path):
        ...
        if event == "pal_attempt_decision_finalized":
            latest_finalized = payload
        elif event == "pal_repair_loop_rejected":
    ...
    if not isinstance(typed_outcome, dict):
        typed_outcome = {}
    summary: dict[str, Any] = {
        "pal_selected_family": latest_finalized.get("selected_family")
        or latest_finalized.get("family_name"),
        "pal_generation_source": latest_finalized.get("generation_source"),
        "pal_materialization_allowed": latest_finalized.get("materialization_allowed"),
        "pal_materialization_denial_reasons": list(
            latest_finalized.get("materialization_denial_reasons") or ()
        ),
        "pal_dangerous_overreach": bool(latest_finalized.get("dangerous_overreach")),
        "pal_tool_result_status": latest_finalized.get("tool_result_status"),
        "pal_tool_result_failure_reason": latest_finalized.get(
            "tool_result_failure_reason"
        ),
        "pal_completion_state": typed_outcome.get("completion_state"),
        "pal_stop_reason": typed_outcome.get("stop_reason"),
        "pal_primary_failure_kind": typed_outcome.get("primary_failure_kind"),
        "pal_plausibility_verdict": typed_outcome.get("plausibility_verdict"),
        "pal_verdict_reasons": list(typed_outcome.get("verdict_reasons") or ()),
        "pal_repair_attempt_count": typed_outcome.get("repair_attempt_count"),
    }
    if latest_rejected:
        summary["pal_last_rejected_verdict"] = latest_rejected.get("last_verdict")
        summary["pal_last_rejected_reasons"] = list(
            latest_rejected.get("last_reasons") or ()
        )
    return summary


def _infer_runner_level_pal_outcome(summary: dict[str, Any]) -> dict[str, Any]:
    if str(summary.get("pal_primary_failure_kind") or "").strip():
        return {}
    ...
    if finish_reason in {
        ...
        return {
            "pal_completion_state": "fail_closed",
            "pal_stop_reason": "runner_infrastructure_failure",
            "pal_primary_failure_kind": "infrastructure_failure",
            "pal_runner_inferred": True,
        }
    return {}
...
def _finalize_summary_row(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    finalized.update(_infer_runner_level_pal_outcome(finalized))
    return finalized
...
def _build_mode_metadata(*, label: str, samples: list[str]) -> dict[str, Any]:
    return {
        ...
        "pal_only_bypass": str(os.environ.get(PAL_ONLY_BYPASS_ENV) or "").strip()
        == "1",
...
def _build_summary_report(
    *,
    ...
    for row in summaries:
        ...
        if bool(row.get("pal_runner_inferred")):
            aggregate["runner_inferred_count"] += 1
        primary_failure_kind = str(row.get("pal_primary_failure_kind") or "").strip()
