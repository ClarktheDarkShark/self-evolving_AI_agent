...
STATE_DIR = str(REPO_ROOT / "outputs" / "sage_runtime" / "tool_state")
TOOL_PATH = (
    REPO_ROOT
    / "outputs"
    / "sage_runtime"
    / "tool_library"
    / "generated_tools"
    / f"{SAGE_BENCHMARK_BRIDGE_TOOL_NAME}.py"
)


def ensure_bridge_macro_tool() -> None:
    TOOL_PATH.parent.mkdir(parents=True, exist_ok=True)
    if TOOL_PATH.exists():
        return
    TOOL_PATH.write_text(build_sage_benchmark_bridge_tool_code(), encoding="utf-8")
...
def build_empty_bridge_session(sample_index: str) -> Session:
    ...
    session.chat_history.inject(
        {
            "role": Role.AGENT,
            "content": (
                'Action: execute_macro("sage_benchmark_bridge_macro", '
                '{"sage_artifact_type": "empty", "sage_artifact_value": [], '
                '"sage_artifact_source": "raw_execution", '
                '"sage_artifact_diagnostics": {"binding_count": 0}, '
    ...
    session.tool_invoked = ["sage_benchmark_bridge_macro"]
    ...
    session.chat_history.inject(
        {
            ...
            "content": (
                'Action: execute_macro("sage_benchmark_bridge_macro", '
                '{"sage_artifact_type": "entity_id", '
                '"sage_artifact_value": "m.02h8b9t", '
                '"sage_artifact_source": "raw_execution", '
                '"sage_artifact_diagnostics": {"binding_count": 1}, '
    ...
    session.chat_history.inject(
        {
            ...
            "content": (
                ...
                "Final variable: #0\n"
                "Semantic: entity result returned by the SAGE query\n"
                "Solves task: yes\n"
                "Trusted final: yes\n"
